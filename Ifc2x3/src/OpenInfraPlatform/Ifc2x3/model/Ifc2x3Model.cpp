...
#include "Ifc2x3Model.h"

namespace OpenInfraPlatform
{
    namespace Ifc2x3
    {
        Ifc2x3Model::Ifc2x3Model()
        {
            m_unit_converter = shared_ptr<UnitConverter>( new UnitConverter() );
            initFileHeader( "Ifc2x3-export.ifc" );
        }
        ...
        void Ifc2x3Model::initIfcModel()
        {
            ...
            app->m_ApplicationFullName = shared_ptr<IfcLabel>( new IfcLabel( "Ifc2x3" ) );
            app->m_ApplicationIdentifier = shared_ptr<IfcIdentifier>( new IfcIdentifier( "Ifc2x3" ) );
            insertEntity(app);
            ...
            project->m_Name = shared_ptr<IfcLabel>(new IfcLabel( "Ifc2x3 project" ) );
        ...
        shared_ptr<IfcProject> Ifc2x3Model::getIfcProject()
        {
            return m_ifc_project;
        }
        shared_ptr<IfcGeometricRepresentationContext> Ifc2x3Model::getIfcGeometricRepresentationContext3D()
        {
            return m_geom_context_3d;
        }

        void Ifc2x3Model::setIfcProject( shared_ptr<IfcProject> project )
        {
            m_ifc_project = project;
        }

        void Ifc2x3Model::insertEntity( shared_ptr<Ifc2x3Entity> e, bool overwrite_existing )
        {
            ...
            std::map<int,shared_ptr<Ifc2x3Entity> >::iterator lb = m_map_entities.lower_bound( entity_id );
            ...
            if( lb != m_map_entities.end() && !(m_map_entities.key_comp()(entity_id, lb->first)) )
            {
                ...
                else
                {
                    ...
                    strs << "Ifc2x3Model::insertEntity: entity with id " << entity_id << " already exists in model" << std::endl;
                    throw Ifc2x3Exception( strs.str().c_str() );
                }
            }
            else
            {
                // the key does not exist in the map
                m_map_entities.insert( lb, std::map<int,shared_ptr<Ifc2x3Entity> >::value_type( entity_id, e ) );
            }

            // TODO: if type is IfcRoot (or subtype), and GlobalID not set, create one
        }

        void Ifc2x3Model::removeEntity( shared_ptr<Ifc2x3Entity> e )
        {
            int remove_id = e->getId();
            std::map<int,shared_ptr<Ifc2x3Entity> >::iterator it_find = m_map_entities.find(remove_id);
            if( it_find != m_map_entities.end() )
            {
                shared_ptr<Ifc2x3Entity> entity_found( it_find->second );
                entity_found->unlinkSelf();
                m_map_entities.erase( it_find );
            }
        }

        void Ifc2x3Model::removeEntity( int entity_id )
        {
            std::map<int,shared_ptr<Ifc2x3Entity> >::iterator it_find = m_map_entities.find(entity_id);
            if( it_find != m_map_entities.end() )
            {
                shared_ptr<Ifc2x3Entity> entity_found( it_find->second );
                entity_found->unlinkSelf();
        ...
        int Ifc2x3Model::getMaxUsedEntityId()
        {
            ...
            std::map<int,shared_ptr<Ifc2x3Entity> >::iterator it_entities = m_map_entities.end();
            --it_entities;
        ...
        void Ifc2x3Model::initFileHeader( std::string file_name )
        {
			std::stringstream strs;
        ...
        void Ifc2x3Model::setFileHeader( std::string header )
        {
            m_file_header = header;
        }

        void Ifc2x3Model::setFileSchema( std::string schema )
        {
            m_file_schema = schema;
        }

        void Ifc2x3Model::clearIfcModel()
        {
            m_ifc_project.reset();
            m_geom_context_3d.reset();

            std::map<int,shared_ptr<Ifc2x3Entity> >::iterator it_entities;
            for( it_entities = m_map_entities.begin(); it_entities != m_map_entities.end(); ++it_entities )
            {
                shared_ptr<Ifc2x3Entity> entity( it_entities->second );
                entity->unlinkSelf();
            }
            m_map_entities.clear();
        }

        void Ifc2x3Model::resetIfcModel()
        {
            clearIfcModel();
            initIfcModel();
            updateCache();
        ...
        void Ifc2x3Model::updateCache()
        {
            bool found_project = false;
            bool found_geom_context = false;

            // try to find IfcProject and IfcGeometricRepresentationContext
            std::map<int,shared_ptr<Ifc2x3Entity> >::iterator it;
            std::map<int,shared_ptr<Ifc2x3Entity> >::iterator it_end = m_map_entities.end();
        ...
        void Ifc2x3Model::clearCache()
        {
            m_ifc_project.reset();
            m_geom_context_3d.reset();
        }

        void Ifc2x3Model::resolveInverseAttributes()
        {
