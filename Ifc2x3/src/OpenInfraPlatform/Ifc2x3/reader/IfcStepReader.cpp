...
namespace OpenInfraPlatform
{
    namespace Ifc2x3
    {
        static std::unordered_map<std::string,Ifc2x3EntityEnum> map_string2entity_enum(initializers_Ifc2x3_entity, initializers_Ifc2x3_entity + sizeof(initializers_Ifc2x3_entity)/sizeof(initializers_Ifc2x3_entity[0]));
        void applyBackwardCompatibility( Ifc2x3Model::IfcVersion backward_version, Ifc2x3EntityEnum type_enum, std::vector<std::string>& args );
        ...
        void readStepLine( const std::string& line, shared_ptr<Ifc2x3Entity>& entity )
        {
            ...
            if( keyword.size() > 0 )
            {
                //std::map<std::string,Ifc2x3EntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                std::unordered_map<std::string,Ifc2x3EntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                ...
                {
                    Ifc2x3EntityEnum entity_enum = it_entity_enum->second;

                    shared_ptr<Ifc2x3Entity> obj( createIfc2x3Entity( entity_enum ) );
                    if( obj )
                    {
                        obj->setId( id );
                        entity = obj;
        ...
        void IfcStepReader::readStreamHeader( std::string& read_in, shared_ptr<Ifc2x3Model> model )
        {
            ...
            if( file_schema_begin != std::string::npos )
            {
                ...
                if( file_schema_end != std::string::npos )
                {
					...
					{
						// hack: assume Ifc2x3
						m_file_schema = "Ifc2x3";
					}

                    model->setFileSchema( m_file_schema );
                    if( m_file_schema.substr(0,6).compare("IFC2X2") == 0 )
                    {
                        m_ifc_version = Ifc2x3Model::IFC2X2;
                    }
                    else if( m_file_schema.substr(0,6).compare("IFC2X3") == 0 )
                    {
                        m_ifc_version = Ifc2x3Model::IFC2X3;
                    }
                    else if( m_file_schema.substr(0,6).compare("IFC2X4") == 0 )
                    {
                        m_ifc_version = Ifc2x3Model::IFC2X4;
                    }
                    else if( m_file_schema.substr(0,5).compare("IFC2X") == 0 )
                    {
                        m_ifc_version = Ifc2x3Model::IFC2X;
                    }
                    else if( m_file_schema.compare("Ifc2x3") == 0 )
                    {
                        m_ifc_version = Ifc2x3Model::Ifc2x3;
                    }
                    else if( m_file_schema.compare("Ifc2x3RC4") == 0 )
                    {
                        m_ifc_version = Ifc2x3Model::Ifc2x3;
                    }
                    else
                    {
                        m_ifc_version = Ifc2x3Model::UNKNOWN;
                    }
                }
            }
        }
        ...
        void IfcStepReader::readStepLines( const std::vector<std::string>& step_lines, std::vector<shared_ptr<Ifc2x3Entity> >& target_entity_vec )
        {
...
#ifdef Ifc2x3_OPENMP
            std::vector<shared_ptr<Ifc2x3Entity> >* target_vec_ptr = &target_entity_vec;
            ...
            #pragma omp parallel firstprivate(num_lines) shared(target_vec_ptr,unkown_entities_ptr)
            {
                ...
                for( int i=0; i<num_lines; ++i )
                {
                    ...
                    try
                    {
                        shared_ptr<Ifc2x3Entity> entity;
        ...
        void IfcStepReader::readEntityArguments( const std::vector<shared_ptr<Ifc2x3Entity> >& vec_entities, const std::map<int,shared_ptr<Ifc2x3Entity> > map_entities )
        {
            ...
            const std::map<int,shared_ptr<Ifc2x3Entity> >* map_ptr = &map_entities;
            const std::vector<shared_ptr<Ifc2x3Entity> >* vec_entities_ptr = &vec_entities;

            #pragma omp parallel firstprivate(num_objects) shared(map_ptr,vec_entities_ptr)
            {
                const std::map<int,shared_ptr<Ifc2x3Entity> > &map_loc = *map_ptr;
                std::stringstream thread_err;

                #pragma omp for schedule(dynamic, 100)
                for( int i=0; i<num_objects; ++i )
                {
                    shared_ptr<Ifc2x3Entity> entity = vec_entities_ptr->at(i);
                    std::string& argument_str = entity->m_arguments;
                    std::vector<std::string> arguments;
                    tokenizeEntityArguments( argument_str, arguments );

                    if( m_ifc_version != Ifc2x3Model::Ifc2x3 )
                    {
                        if( m_ifc_version != Ifc2x3Model::UNDEFINED && m_ifc_version != Ifc2x3Model::UNKNOWN )
                        {
                            Ifc2x3EntityEnum entity_enum = entity->m_entity_enum;
            ...
            std::vector<shared_ptr<Ifc2x3Entity> >::const_iterator it_entity_vec = vec_entities.begin();
            int i=0;
            for( ; it_entity_vec!=vec_entities.end(); ++it_entity_vec )
            {
                shared_ptr<Ifc2x3Entity> entity = (*it_entity_vec);
            ...
            if( err.tellp() > 0 )
            {
                throw Ifc2x3Exception( err.str() );
            }
        }

        void IfcStepReader::readStreamData(	std::string& read_in, std::map<int,shared_ptr<Ifc2x3Entity> >& target_map )
        {
            ...
            std::vector<shared_ptr<Ifc2x3Entity> > vec_entities;
            try
            {
        ...
        void applyBackwardCompatibility( Ifc2x3Model::IfcVersion version, Ifc2x3EntityEnum type_enum, std::vector<std::string>& args )
        {
            // TODO: replace this workaround with a systematic backward compatibility, possibly generated from schema diff
            if( version < Ifc2x3Model::IFC2X )
            {
                throw Ifc2x3Exception( "Unsupported IFC version" );
            }
            if( version <= Ifc2x3Model::IFC2X )
            {
