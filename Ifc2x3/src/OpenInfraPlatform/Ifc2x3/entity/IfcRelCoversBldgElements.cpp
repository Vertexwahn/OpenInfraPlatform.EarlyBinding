...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcRelCoversBldgElements::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcRelCoversBldgElements::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelCoversBldgElements, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelCoversBldgElements::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
			IfcRelConnects::setInverseCounterparts( ptr_self_entity );
			shared_ptr<IfcRelCoversBldgElements> ptr_self = dynamic_pointer_cast<IfcRelCoversBldgElements>( ptr_self_entity );
			if( !ptr_self ) { throw Ifc2x3Exception( "IfcRelCoversBldgElements::setInverseCounterparts: type mismatch" ); }
			for( int i=0; i<m_RelatedCoverings.size(); ++i )
			{
				if( m_RelatedCoverings[i] )
				{
					m_RelatedCoverings[i]->m_Covers_inverse.push_back( ptr_self );
				}
			}
		...
		void IfcRelCoversBldgElements::unlinkSelf()
		{
			...
			for( int i=0; i<m_RelatedCoverings.size(); ++i )
			{
				if( m_RelatedCoverings[i] )
				{
					std::vector<weak_ptr<IfcRelCoversBldgElements> >& Covers_inverse = m_RelatedCoverings[i]->m_Covers_inverse;
					std::vector<weak_ptr<IfcRelCoversBldgElements> >::iterator it_Covers_inverse;
					for( it_Covers_inverse = Covers_inverse.begin(); it_Covers_inverse != Covers_inverse.end(); ++it_Covers_inverse)
					{
						shared_ptr<IfcRelCoversBldgElements> self_candidate( *it_Covers_inverse );
						if( self_candidate->getId() == this->getId() )
						{
							Covers_inverse.erase( it_Covers_inverse );
							break;
						}
					}
				}
