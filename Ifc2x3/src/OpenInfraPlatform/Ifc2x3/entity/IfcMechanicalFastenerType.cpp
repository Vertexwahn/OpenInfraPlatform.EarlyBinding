...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcMechanicalFastenerType::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
			shared_ptr<IfcMechanicalFastenerType> other = dynamic_pointer_cast<IfcMechanicalFastenerType>(other_entity);
			if( !other) { return; }
		...
		void IfcMechanicalFastenerType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcMechanicalFastenerType, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			if( num_args>9 ){ std::cout << "Wrong parameter count for entity IfcMechanicalFastenerType, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; }
			#endif
		...
		void IfcMechanicalFastenerType::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
			IfcFastenerType::setInverseCounterparts( ptr_self_entity );
		}
		void IfcMechanicalFastenerType::unlinkSelf()
		{
			IfcFastenerType::unlinkSelf();
		}
