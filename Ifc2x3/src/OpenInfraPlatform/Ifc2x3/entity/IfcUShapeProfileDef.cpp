...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcUShapeProfileDef::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcUShapeProfileDef::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcUShapeProfileDef, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			if( num_args>11 ){ std::cout << "Wrong parameter count for entity IfcUShapeProfileDef, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; }
			#endif
			...
			m_EdgeRadius = IfcPositiveLengthMeasure::readStepData( args[8] );
			m_FlangeSlope = IfcPlaneAngleMeasure::readStepData( args[9] );
			m_CentreOfGravityInX = IfcPositiveLengthMeasure::readStepData( args[10] );
		}
		void IfcUShapeProfileDef::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
