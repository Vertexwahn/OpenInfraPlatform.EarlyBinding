...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcTelecomAddress::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcTelecomAddress::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTelecomAddress, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			if( num_args>8 ){ std::cout << "Wrong parameter count for entity IfcTelecomAddress, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; }
			#endif
			...
			m_WWWHomePageURL = IfcLabel::readStepData( args[7] );
		}
		void IfcTelecomAddress::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
