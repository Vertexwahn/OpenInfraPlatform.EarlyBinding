...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcWasteTerminalTypeEnum::getStepParameter( std::stringstream& stream, bool is_select_type ) const
		{
			...
			else if( m_enum == ENUM_GREASEINTERCEPTOR )
			{
				stream << ".GREASEINTERCEPTOR.";
			}
			else if( m_enum == ENUM_OILINTERCEPTOR )
			{
				stream << ".OILINTERCEPTOR.";
			}
			else if( m_enum == ENUM_PETROLINTERCEPTOR )
			{
				stream << ".PETROLINTERCEPTOR.";
			}
		...
		shared_ptr<IfcWasteTerminalTypeEnum> IfcWasteTerminalTypeEnum::readStepData( std::string& arg )
		{
			...
			else if( _stricmp( arg.c_str(), ".GREASEINTERCEPTOR." ) == 0 )
			{
				type_object->m_enum = IfcWasteTerminalTypeEnum::ENUM_GREASEINTERCEPTOR;
			}
			else if( _stricmp( arg.c_str(), ".OILINTERCEPTOR." ) == 0 )
			{
				type_object->m_enum = IfcWasteTerminalTypeEnum::ENUM_OILINTERCEPTOR;
			}
			else if( _stricmp( arg.c_str(), ".PETROLINTERCEPTOR." ) == 0 )
			{
				type_object->m_enum = IfcWasteTerminalTypeEnum::ENUM_PETROLINTERCEPTOR;
			}
