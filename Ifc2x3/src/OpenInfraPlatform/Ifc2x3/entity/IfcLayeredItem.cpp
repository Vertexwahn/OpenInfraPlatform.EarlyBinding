...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		IfcLayeredItem::~IfcLayeredItem() {}
		shared_ptr<IfcLayeredItem> IfcLayeredItem::readStepData( std::string& arg, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X3::IfcLayeredItem::readStepData" << std::endl;
				throw Ifc2x3Exception( strs.str() );
			}
			return shared_ptr<IfcLayeredItem>();
		}
