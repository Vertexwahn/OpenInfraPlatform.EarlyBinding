...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcPresentationLayerWithStyle::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcPresentationLayerWithStyle::getStepLine( std::stringstream& stream ) const
		{
			...
			stream << ",";
			writeTypeList( stream, m_LayerStyles, true );
			stream << ");";
		}
		void IfcPresentationLayerWithStyle::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
		void IfcPresentationLayerWithStyle::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPresentationLayerWithStyle, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			...
			readSelectList( args[7], m_LayerStyles, map );
		}
		void IfcPresentationLayerWithStyle::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
