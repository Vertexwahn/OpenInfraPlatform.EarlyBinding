...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		IfcMeasureValue::~IfcMeasureValue() {}
		shared_ptr<IfcMeasureValue> IfcMeasureValue::readStepData( std::string& arg, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				if( keyword.compare("IFCVOLUMEMEASURE")== 0 )
				{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X3::IfcMeasureValue::readStepData" << std::endl;
				throw Ifc2x3Exception( strs.str() );
			}
			return shared_ptr<IfcMeasureValue>();
		}
