...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		IfcDerivedMeasureValue::~IfcDerivedMeasureValue() {}
		shared_ptr<IfcDerivedMeasureValue> IfcDerivedMeasureValue::readStepData( std::string& arg, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				if( keyword.compare("IFCVOLUMETRICFLOWRATEMEASURE")== 0 )
				{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X3::IfcDerivedMeasureValue::readStepData" << std::endl;
				throw Ifc2x3Exception( strs.str() );
			}
			return shared_ptr<IfcDerivedMeasureValue>();
		}
