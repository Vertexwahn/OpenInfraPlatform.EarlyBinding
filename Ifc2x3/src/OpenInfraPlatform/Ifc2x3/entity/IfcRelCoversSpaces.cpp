...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcRelCoversSpaces::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
			...
			m_RelatedSpace = other->m_RelatedSpace;
		...
		void IfcRelCoversSpaces::getStepLine( std::stringstream& stream ) const
		{
			...
			stream << ",";
			if( m_RelatedSpace ) { stream << "#" << m_RelatedSpace->getId(); } else { stream << "$"; }
			stream << ",";
		...
		void IfcRelCoversSpaces::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelCoversSpaces, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			...
			readEntityReference( args[4], m_RelatedSpace, map );
			readEntityReferenceList( args[5], m_RelatedCoverings, map );
		}
		void IfcRelCoversSpaces::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
			IfcRelConnects::setInverseCounterparts( ptr_self_entity );
			shared_ptr<IfcRelCoversSpaces> ptr_self = dynamic_pointer_cast<IfcRelCoversSpaces>( ptr_self_entity );
			if( !ptr_self ) { throw Ifc2x3Exception( "IfcRelCoversSpaces::setInverseCounterparts: type mismatch" ); }
			...
			if( m_RelatedSpace )
			{
				m_RelatedSpace->m_HasCoverings_inverse.push_back( ptr_self );
			}
		}
		void IfcRelCoversSpaces::unlinkSelf()
		{
			...
			if( m_RelatedSpace )
			{
				std::vector<weak_ptr<IfcRelCoversSpaces> >& HasCoverings_inverse = m_RelatedSpace->m_HasCoverings_inverse;
