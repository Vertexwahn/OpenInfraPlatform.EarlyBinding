...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcRoleEnum::getStepParameter( std::stringstream& stream, bool is_select_type ) const
		{
			...
			else if( m_enum == ENUM_COMISSIONINGENGINEER )
			{
				stream << ".COMISSIONINGENGINEER.";
			}
		...
		shared_ptr<IfcRoleEnum> IfcRoleEnum::readStepData( std::string& arg )
		{
			...
			else if( _stricmp( arg.c_str(), ".COMISSIONINGENGINEER." ) == 0 )
			{
				type_object->m_enum = IfcRoleEnum::ENUM_COMISSIONINGENGINEER;
			}
