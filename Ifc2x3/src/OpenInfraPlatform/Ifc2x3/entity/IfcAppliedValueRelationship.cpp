...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcAppliedValueRelationship::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcAppliedValueRelationship::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<5 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcAppliedValueRelationship, expecting 5, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			...
			#endif
			readEntityReference( args[0], m_ComponentOfTotal, map );
			readEntityReferenceList( args[1], m_Components, map );
			m_ArithmeticOperator = IfcArithmeticOperatorEnum::readStepData( args[2] );
			m_Name = IfcLabel::readStepData( args[3] );
			m_Description = IfcText::readStepData( args[4] );
		}
		void IfcAppliedValueRelationship::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
			shared_ptr<IfcAppliedValueRelationship> ptr_self = dynamic_pointer_cast<IfcAppliedValueRelationship>( ptr_self_entity );
			if( !ptr_self ) { throw Ifc2x3Exception( "IfcAppliedValueRelationship::setInverseCounterparts: type mismatch" ); }
