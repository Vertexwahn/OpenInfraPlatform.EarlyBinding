...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcAsset::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcAsset::getStepLine( std::stringstream& stream ) const
		{
			...
			stream << ",";
			if( m_AssetID ) { m_AssetID->getStepParameter( stream ); } else { stream << "$"; }
			stream << ",";
			...
			stream << ",";
			if( m_IncorporationDate ) { stream << "#" << m_IncorporationDate->getId(); } else { stream << "$"; }
			stream << ",";
		...
		void IfcAsset::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<14 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcAsset, expecting 14, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			...
			m_AssetID = IfcIdentifier::readStepData( args[5] );
			...
			readEntityReference( args[12], m_IncorporationDate, map );
			readEntityReference( args[13], m_DepreciatedValue, map );
		}
		void IfcAsset::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
