...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcRoof::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
			shared_ptr<IfcRoof> other = dynamic_pointer_cast<IfcRoof>(other_entity);
			if( !other) { return; }
		...
		void IfcRoof::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRoof, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			...
			m_ShapeType = IfcRoofTypeEnum::readStepData( args[8] );
		}
		void IfcRoof::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
			IfcBuildingElement::setInverseCounterparts( ptr_self_entity );
		}
		void IfcRoof::unlinkSelf()
		{
