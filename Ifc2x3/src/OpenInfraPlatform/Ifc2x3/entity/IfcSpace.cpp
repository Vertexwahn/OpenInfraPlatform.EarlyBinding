...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcSpace::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
			...
			m_InteriorOrExteriorSpace = other->m_InteriorOrExteriorSpace;
		...
		void IfcSpace::getStepLine( std::stringstream& stream ) const
		{
			...
			stream << ",";
			if( m_InteriorOrExteriorSpace ) { m_InteriorOrExteriorSpace->getStepParameter( stream ); } else { stream << "$"; }
			stream << ",";
		...
		void IfcSpace::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcSpace, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			...
			m_InteriorOrExteriorSpace = IfcInternalOrExternalEnum::readStepData( args[9] );
			m_ElevationWithFlooring = IfcLengthMeasure::readStepData( args[10] );
		}
		void IfcSpace::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
