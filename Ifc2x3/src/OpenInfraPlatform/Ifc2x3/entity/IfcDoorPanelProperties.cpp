...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcDoorPanelProperties::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcDoorPanelProperties::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcDoorPanelProperties, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcDoorPanelProperties::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
