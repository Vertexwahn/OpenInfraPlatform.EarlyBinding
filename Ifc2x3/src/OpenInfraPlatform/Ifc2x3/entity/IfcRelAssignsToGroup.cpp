...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcRelAssignsToGroup::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcRelAssignsToGroup::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<7 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelAssignsToGroup, expecting 7, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelAssignsToGroup::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw Ifc2x3Exception( "IfcRelAssignsToGroup::setInverseCounterparts: type mismatch" ); }
			if( m_RelatingGroup )
			{
				m_RelatingGroup->m_IsGroupedBy_inverse = ptr_self;
			}
		}
		void IfcRelAssignsToGroup::unlinkSelf()
		{
			...
			if( m_RelatingGroup )
			{
				shared_ptr<IfcRelAssignsToGroup> self_candidate( m_RelatingGroup->m_IsGroupedBy_inverse );
				if( self_candidate->getId() == this->getId() )
				{
					weak_ptr<IfcRelAssignsToGroup>& self_candidate_weak = m_RelatingGroup->m_IsGroupedBy_inverse;
