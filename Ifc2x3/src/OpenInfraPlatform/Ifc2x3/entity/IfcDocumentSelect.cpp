...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		IfcDocumentSelect::~IfcDocumentSelect() {}
		shared_ptr<IfcDocumentSelect> IfcDocumentSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X3::IfcDocumentSelect::readStepData" << std::endl;
				throw Ifc2x3Exception( strs.str() );
			}
			return shared_ptr<IfcDocumentSelect>();
		}
