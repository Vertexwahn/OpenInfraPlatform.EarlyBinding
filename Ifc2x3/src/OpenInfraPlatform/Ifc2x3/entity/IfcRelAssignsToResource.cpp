...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcRelAssignsToResource::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcRelAssignsToResource::getStepLine( std::stringstream& stream ) const
		{
			...
			stream << ",";
			if( m_RelatingResource ) { stream << "#" << m_RelatingResource->getId(); } else { stream << "$"; }
			stream << ");";
		}
		void IfcRelAssignsToResource::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
		void IfcRelAssignsToResource::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<7 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelAssignsToResource, expecting 7, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			...
			readEntityReference( args[6], m_RelatingResource, map );
		}
		void IfcRelAssignsToResource::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw Ifc2x3Exception( "IfcRelAssignsToResource::setInverseCounterparts: type mismatch" ); }
			if( m_RelatingResource )
			{
				m_RelatingResource->m_ResourceOf_inverse.push_back( ptr_self );
			}
		}
		void IfcRelAssignsToResource::unlinkSelf()
		{
			...
			if( m_RelatingResource )
			{
				std::vector<weak_ptr<IfcRelAssignsToResource> >& ResourceOf_inverse = m_RelatingResource->m_ResourceOf_inverse;
