...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcRelSequence::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcRelSequence::getStepLine( std::stringstream& stream ) const
		{
			...
			stream << ",";
			if( m_TimeLag ) { m_TimeLag->getStepParameter( stream ); } else { stream << "$"; }
			stream << ",";
		...
		void IfcRelSequence::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelSequence, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			if( num_args>8 ){ std::cout << "Wrong parameter count for entity IfcRelSequence, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; }
			#endif
			...
			m_TimeLag = IfcTimeMeasure::readStepData( args[6] );
			m_SequenceType = IfcSequenceEnum::readStepData( args[7] );
		}
		void IfcRelSequence::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
			IfcRelConnects::setInverseCounterparts( ptr_self_entity );
			shared_ptr<IfcRelSequence> ptr_self = dynamic_pointer_cast<IfcRelSequence>( ptr_self_entity );
			if( !ptr_self ) { throw Ifc2x3Exception( "IfcRelSequence::setInverseCounterparts: type mismatch" ); }
			if( m_RelatedProcess )
			{
