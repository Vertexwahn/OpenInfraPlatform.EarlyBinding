...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcReinforcingBar::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcReinforcingBar::getStepLine( std::stringstream& stream ) const
		{
			...
			stream << ",";
			if( m_BarRole ) { m_BarRole->getStepParameter( stream ); } else { stream << "$"; }
			stream << ",";
		...
		void IfcReinforcingBar::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<14 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcReinforcingBar, expecting 14, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			...
			m_BarRole = IfcReinforcingBarRoleEnum::readStepData( args[12] );
			m_BarSurface = IfcReinforcingBarSurfaceEnum::readStepData( args[13] );
		}
		void IfcReinforcingBar::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
