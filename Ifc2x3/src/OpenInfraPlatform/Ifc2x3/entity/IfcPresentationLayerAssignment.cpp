...
namespace OpenInfraPlatform
{
	namespace Ifc2x3
	{
		...
		void IfcPresentationLayerAssignment::setEntity( shared_ptr<Ifc2x3Entity> other_entity )
		{
		...
		void IfcPresentationLayerAssignment::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc2x3Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<4 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPresentationLayerAssignment, expecting 4, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc2x3Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcPresentationLayerAssignment::setInverseCounterparts( shared_ptr<Ifc2x3Entity> ptr_self_entity )
		{
			shared_ptr<IfcPresentationLayerAssignment> ptr_self = dynamic_pointer_cast<IfcPresentationLayerAssignment>( ptr_self_entity );
			if( !ptr_self ) { throw Ifc2x3Exception( "IfcPresentationLayerAssignment::setInverseCounterparts: type mismatch" ); }
			for( int i=0; i<m_AssignedItems.size(); ++i )
			{
				...
				{
					AssignedItems_IfcRepresentationItem->m_LayerAssignments_inverse.push_back( ptr_self );
				}
			}
		}
		void IfcPresentationLayerAssignment::unlinkSelf()
		{
			for( int i=0; i<m_AssignedItems.size(); ++i )
			{
				...
				if( AssignedItems_IfcRepresentationItem )
				{
					std::vector<weak_ptr<IfcPresentationLayerAssignment> >& LayerAssignments_inverse = AssignedItems_IfcRepresentationItem->m_LayerAssignments_inverse;
					std::vector<weak_ptr<IfcPresentationLayerAssignment> >::iterator it_LayerAssignments_inverse;
					for( it_LayerAssignments_inverse = LayerAssignments_inverse.begin(); it_LayerAssignments_inverse != LayerAssignments_inverse.end(); ++it_LayerAssignments_inverse)
					{
						shared_ptr<IfcPresentationLayerAssignment> self_candidate( *it_LayerAssignments_inverse );
