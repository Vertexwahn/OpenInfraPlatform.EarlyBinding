...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcOwnerHistory::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcOwnerHistory::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcOwnerHistory, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcOwnerHistory::setInverseCounterparts( shared_ptr<IfcRoadEntity> )
		{
		}
