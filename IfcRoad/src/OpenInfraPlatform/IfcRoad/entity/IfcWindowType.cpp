...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcWindowType::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcWindowType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<13 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcWindowType, expecting 13, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcWindowType::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
