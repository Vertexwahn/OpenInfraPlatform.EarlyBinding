...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcEngine::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcEngine::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcEngine, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcEngine::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
