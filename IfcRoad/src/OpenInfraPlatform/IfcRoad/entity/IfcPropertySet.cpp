...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcPropertySet::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcPropertySet::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<5 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPropertySet, expecting 5, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcPropertySet::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcRoadException( "IfcPropertySet::setInverseCounterparts: type mismatch" ); }
