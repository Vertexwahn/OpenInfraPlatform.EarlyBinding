...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcTableColumn::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcTableColumn::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<5 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTableColumn, expecting 5, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcTableColumn::setInverseCounterparts( shared_ptr<IfcRoadEntity> )
		{
		}
