...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcCsgPrimitive3D::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcCsgPrimitive3D::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<1 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcCsgPrimitive3D, expecting 1, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcCsgPrimitive3D::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
