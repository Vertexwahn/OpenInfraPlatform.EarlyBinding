...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcAxis2Placement2D::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcAxis2Placement2D::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcAxis2Placement2D, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcAxis2Placement2D::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
