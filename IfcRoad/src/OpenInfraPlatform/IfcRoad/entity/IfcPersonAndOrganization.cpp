...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcPersonAndOrganization::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcPersonAndOrganization::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<3 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPersonAndOrganization, expecting 3, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcPersonAndOrganization::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
			shared_ptr<IfcPersonAndOrganization> ptr_self = dynamic_pointer_cast<IfcPersonAndOrganization>( ptr_self_entity );
			if( !ptr_self ) { throw IfcRoadException( "IfcPersonAndOrganization::setInverseCounterparts: type mismatch" ); }
