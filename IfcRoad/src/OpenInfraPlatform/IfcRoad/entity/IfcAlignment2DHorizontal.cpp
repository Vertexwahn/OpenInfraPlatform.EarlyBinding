...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcAlignment2DHorizontal::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcAlignment2DHorizontal::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcAlignment2DHorizontal, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcAlignment2DHorizontal::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
			shared_ptr<IfcAlignment2DHorizontal> ptr_self = dynamic_pointer_cast<IfcAlignment2DHorizontal>( ptr_self_entity );
			if( !ptr_self ) { throw IfcRoadException( "IfcAlignment2DHorizontal::setInverseCounterparts: type mismatch" ); }
