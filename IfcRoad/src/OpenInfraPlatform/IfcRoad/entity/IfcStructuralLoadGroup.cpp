...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcStructuralLoadGroup::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcStructuralLoadGroup::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcStructuralLoadGroup, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcStructuralLoadGroup::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
