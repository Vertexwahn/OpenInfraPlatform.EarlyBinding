...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcPerson::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcPerson::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPerson, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcPerson::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
			shared_ptr<IfcPerson> ptr_self = dynamic_pointer_cast<IfcPerson>( ptr_self_entity );
			if( !ptr_self ) { throw IfcRoadException( "IfcPerson::setInverseCounterparts: type mismatch" ); }
