...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		...
		void IfcAlignment2DVerticalSegment::setEntity( shared_ptr<IfcRoadEntity> other_entity )
		{
		...
		void IfcAlignment2DVerticalSegment::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcRoadEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<7 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcAlignment2DVerticalSegment, expecting 7, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcRoadException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcAlignment2DVerticalSegment::setInverseCounterparts( shared_ptr<IfcRoadEntity> ptr_self_entity )
		{
