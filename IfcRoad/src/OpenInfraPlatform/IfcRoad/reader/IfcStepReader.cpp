...
#include "OpenInfraPlatform/IfcRoad/model/UnknownEntityException.h"
#include "OpenInfraPlatform/IfcRoad/IfcRoadEntitiesMap.h"
...
namespace OpenInfraPlatform
{
	namespace IfcRoad
	{
		static std::tr1::unordered_map<std::string, IfcRoadEntityEnum> map_string2entity_enum(initializers_IfcRoad_entity, initializers_IfcRoad_entity + sizeof(initializers_IfcRoad_entity) / sizeof(initializers_IfcRoad_entity[0]));
		void applyBackwardCompatibility( IfcRoadModel::IfcVersion backward_version, IfcRoadEntityEnum type_enum, std::vector<std::string>& args );
		void applyBackwardCompatibility( std::string& keyword, std::string& step_line );
		IfcRoadEntity* createIfcRoadEntity( const IfcRoadEntityEnum entity_enum );

		void readStepLine( const std::string& line, shared_ptr<IfcRoadEntity>& entity )
		{
			...
			if( keyword.size() > 0 )
			{
				//std::map<std::string,IfcRoadEntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
				std::tr1::unordered_map<std::string,IfcRoadEntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
				...
				{
					IfcRoadEntityEnum entity_enum = it_entity_enum->second;

					shared_ptr<IfcRoadEntity> obj( createIfcRoadEntity( entity_enum ) );
					if( obj )
					{
						obj->setId( id );
						entity = obj;
		...
		void IfcStepReader::readStreamHeader( std::string& read_in, shared_ptr<IfcRoadModel> model )
		{
			...
			if( file_schema_begin != std::string::npos )
			{
				...
				if( file_schema_end != std::string::npos )
				{
					...
					{
						m_ifc_version = IfcRoadModel::UNKNOWN;
					}
				}
			}
		}
		...
		void IfcStepReader::readStepLines( const std::vector<std::string>& step_lines, std::vector<shared_ptr<IfcRoadEntity> >& target_entity_vec )
		{
...
#ifdef IfcAlignment_OPENMP
			std::vector<shared_ptr<IfcRoadEntity> >* target_vec_ptr = &target_entity_vec;
			...
			#pragma omp parallel firstprivate(num_lines) shared(target_vec_ptr,unkown_entities_ptr)
			{
				...
				for( int i=0; i<num_lines; ++i )
				{
					...
					try
					{
						shared_ptr<IfcRoadEntity> entity;
		...
		void IfcStepReader::readEntityArguments( const std::vector<shared_ptr<IfcRoadEntity> >& vec_entities, const std::map<int,shared_ptr<IfcRoadEntity> > map_entities )
		{
			...
			const std::map<int,shared_ptr<IfcRoadEntity> >* map_ptr = &map_entities;
			const std::vector<shared_ptr<IfcRoadEntity> >* vec_entities_ptr = &vec_entities;

			#pragma omp parallel firstprivate(num_objects) shared(map_ptr,vec_entities_ptr)
			{
				const std::map<int,shared_ptr<IfcRoadEntity> > &map_loc = *map_ptr;
				std::stringstream thread_err;

				#pragma omp for schedule(dynamic, 100)
				for( int i=0; i<num_objects; ++i )
				{
					shared_ptr<IfcRoadEntity> entity = vec_entities_ptr->at(i);
					std::string& argument_str = entity->m_arguments;
					std::vector<std::string> arguments;
					tokenizeEntityArguments( argument_str, arguments );

					if( m_ifc_version != IfcRoadModel::IFC4 )
					{
						if( m_ifc_version != IfcRoadModel::UNDEFINED && m_ifc_version != IfcRoadModel::UNKNOWN )
						{
							IfcRoadEntityEnum entity_enum = entity->m_entity_enum;
			...
			std::vector<shared_ptr<IfcRoadEntity> >::const_iterator it_entity_vec = vec_entities.begin();
			int i=0;
			for( ; it_entity_vec!=vec_entities.end(); ++it_entity_vec )
			{
				shared_ptr<IfcRoadEntity> entity = (*it_entity_vec);
				std::string& argument_str = entity->m_arguments;
				std::vector<std::string> arguments;
				tokenizeEntityArguments( argument_str, arguments );
			...
			if( err.tellp() > 0 )
			{
				throw IfcRoadException( err.str() );
			}
		}

		void IfcStepReader::readStreamData(	std::string& read_in, std::map<int,shared_ptr<IfcRoadEntity> >& target_map )
		{
			...
			std::vector<shared_ptr<IfcRoadEntity> > vec_entities;
			try
			{
		...
		void applyBackwardCompatibility( IfcRoadModel::IfcVersion version, IfcRoadEntityEnum type_enum, std::vector<std::string>& args )
		{
			// TODO: replace this workaround with a systematic backward compatibility, possibly generated from schema diff
			if( version < IfcRoadModel::IFC2X )
			{
				throw IfcRoadException( "Unsupported IFC version" );
			}
			if( version <= IfcRoadModel::IFC2X )
			{
			...
			if( version <= IfcRoadModel::IFC2X2 )
			{
				switch( type_enum )
				{
				case IFCCOLOURRGB:
			...
			if( version <= IfcRoadModel::IFC2X3 )
			{
				switch( type_enum )
				{
				// B
