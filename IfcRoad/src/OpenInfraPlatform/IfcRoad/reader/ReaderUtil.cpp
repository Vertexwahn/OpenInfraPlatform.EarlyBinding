    ...
    namespace IfcRoad
    {
        ...
        void checkOpeningClosingParenthesis( const char* ch_check )
        {
            ...
            if( num_opening != num_closing )
            {
                ...
                throw IfcRoadException( err.str().c_str(), __func__ );
            }
        }
        ...
        void tokenizeList( std::string& list_str, std::vector<std::string>& list_items )
        {
            ...
            while( *stream_pos != '\0' )
            {
                ...
                if( *stream_pos == ',' )
                {
                    ...
                    if( *stream_pos == '\0' )
                    {
                        throw IfcRoadException( "tokenizeList: *stream_pos == '\0'", __func__ );
                    }
                    continue;
                }
                ++stream_pos;
        ...
        void tokenizeInlineArgument( std::string arg, std::string& keyword, std::string& inline_arg )
        {
            if( arg.size() == 0 )
            {
                throw IfcRoadException( "tokenizeInlineArgument: arg.size() == 0", __func__ );
            }
            if( arg.compare("$") == 0 )
            {
                return;
            ...
            if( arg.at(0) == '#' )
            {
                throw IfcRoadException( "tokenizeInlineArgument: argument begins with #, so it is not inline", __func__ );
            }
