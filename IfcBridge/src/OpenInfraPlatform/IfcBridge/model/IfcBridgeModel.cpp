...
#include "IfcBridgeModel.h"

namespace OpenInfraPlatform
{
    namespace IfcBridge
    {
        IfcBridgeModel::IfcBridgeModel()
        {
            m_unit_converter = shared_ptr<UnitConverter>( new UnitConverter() );
            initFileHeader( "IfcBridge-export.ifc" );
        }
        ...
        void IfcBridgeModel::initIfcModel()
        {
            clearIfcModel();
            ...
            app->m_ApplicationFullName = shared_ptr<IfcLabel>( new IfcLabel( "IfcBridge" ) );
            app->m_ApplicationIdentifier = shared_ptr<IfcIdentifier>( new IfcIdentifier( "IfcBridge" ) );
            insertEntity(app);
            ...
            project->m_Name = shared_ptr<IfcLabel>(new IfcLabel( "IfcBridge project" ) );
        ...
        shared_ptr<IfcProject> IfcBridgeModel::getIfcProject()
        {
            return m_ifc_project;
        }
        shared_ptr<IfcGeometricRepresentationContext> IfcBridgeModel::getIfcGeometricRepresentationContext3D()
        {
            return m_geom_context_3d;
        }

        void IfcBridgeModel::setIfcProject( shared_ptr<IfcProject> project )
        {
            m_ifc_project = project;
        }

        void IfcBridgeModel::insertEntity( shared_ptr<IfcBridgeEntity> e, bool overwrite_existing )
        {
            ...
            std::map<int,shared_ptr<IfcBridgeEntity> >::iterator lb = m_map_entities.lower_bound( entity_id );
            ...
            if( lb != m_map_entities.end() && !(m_map_entities.key_comp()(entity_id, lb->first)) )
            {
                ...
                else
                {
                    ...
                    strs << "IfcBridgeModel::insertEntity: entity with id " << entity_id << " already exists in model" << std::endl;
                    throw IfcBridgeException( strs.str().c_str() );
                }
            }
            else
            {
                // the key does not exist in the map
                m_map_entities.insert( lb, std::map<int,shared_ptr<IfcBridgeEntity> >::value_type( entity_id, e ) );
            }

            // TODO: if type is IfcRoot (or subtype), and GlobalID not set, create one
        }

        void IfcBridgeModel::removeEntity( shared_ptr<IfcBridgeEntity> e )
        {
            int remove_id = e->getId();
            std::map<int,shared_ptr<IfcBridgeEntity> >::iterator it_find = m_map_entities.find(remove_id);
            if( it_find != m_map_entities.end() )
            {
                shared_ptr<IfcBridgeEntity> entity_found( it_find->second );
                entity_found->unlinkSelf();
                m_map_entities.erase( it_find );
            }
        }

        void IfcBridgeModel::removeEntity( int entity_id )
        {
            std::map<int,shared_ptr<IfcBridgeEntity> >::iterator it_find = m_map_entities.find(entity_id);
            if( it_find != m_map_entities.end() )
            {
                shared_ptr<IfcBridgeEntity> entity_found( it_find->second );
                entity_found->unlinkSelf();
        ...
        int IfcBridgeModel::getMaxUsedEntityId()
        {
            ...
            std::map<int,shared_ptr<IfcBridgeEntity> >::iterator it_entities = m_map_entities.end();
            --it_entities;
        ...
        void IfcBridgeModel::initFileHeader( std::string file_name )
        {
            std::stringstream strs;
        ...
        void IfcBridgeModel::setFileHeader( std::string header )
        {
            m_file_header = header;
        }

        void IfcBridgeModel::setFileSchema( std::string schema )
        {
            m_file_schema = schema;
        }

        void IfcBridgeModel::clearIfcModel()
        {
            m_ifc_project.reset();
            m_geom_context_3d.reset();

            std::map<int,shared_ptr<IfcBridgeEntity> >::iterator it_entities;
            for( it_entities = m_map_entities.begin(); it_entities != m_map_entities.end(); ++it_entities )
            {
                shared_ptr<IfcBridgeEntity> entity( it_entities->second );
                entity->unlinkSelf();
            }
            m_map_entities.clear();
        }

        void IfcBridgeModel::resetIfcModel()
        {
            clearIfcModel();
            initIfcModel();
            updateCache();
        ...
        void IfcBridgeModel::updateCache()
        {
            bool found_project = false;
            bool found_geom_context = false;

            // try to find IfcProject and IfcGeometricRepresentationContext
            std::map<int,shared_ptr<IfcBridgeEntity> >::iterator it;
            std::map<int,shared_ptr<IfcBridgeEntity> >::iterator it_end = m_map_entities.end();
            for( it=m_map_entities.begin(); it!=it_end; ++it )
            {
                shared_ptr<IfcBridgeEntity> obj = it->second;
        ...
        void IfcBridgeModel::clearCache()
        {
            m_ifc_project.reset();
            m_geom_context_3d.reset();
        }

        void IfcBridgeModel::resolveInverseAttributes()
        {
