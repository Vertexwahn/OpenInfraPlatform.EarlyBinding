...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcFillStyleSelect::~IfcFillStyleSelect() {}
		shared_ptr<IfcFillStyleSelect> IfcFillStyleSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcFillStyleSelect::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcFillStyleSelect>();
		}
