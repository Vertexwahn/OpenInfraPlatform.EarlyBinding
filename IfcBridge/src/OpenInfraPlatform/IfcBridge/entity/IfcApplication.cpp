...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcApplication::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcApplication::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<4 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcApplication, expecting 4, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcApplication::setInverseCounterparts( shared_ptr<IfcBridgeEntity> )
		{
		}
