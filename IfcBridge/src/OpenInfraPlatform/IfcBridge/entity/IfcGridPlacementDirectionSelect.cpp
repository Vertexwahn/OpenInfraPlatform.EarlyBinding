...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcGridPlacementDirectionSelect::~IfcGridPlacementDirectionSelect() {}
		shared_ptr<IfcGridPlacementDirectionSelect> IfcGridPlacementDirectionSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcGridPlacementDirectionSelect::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcGridPlacementDirectionSelect>();
		}
