...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcMetricValueSelect::~IfcMetricValueSelect() {}
		shared_ptr<IfcMetricValueSelect> IfcMetricValueSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcMetricValueSelect::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcMetricValueSelect>();
		}
