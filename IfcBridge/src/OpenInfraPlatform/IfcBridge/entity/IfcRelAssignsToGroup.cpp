...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcRelAssignsToGroup::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcRelAssignsToGroup::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<7 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelAssignsToGroup, expecting 7, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelAssignsToGroup::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcBridgeException( "IfcRelAssignsToGroup::setInverseCounterparts: type mismatch" ); }
			if( m_RelatingGroup )
			{
