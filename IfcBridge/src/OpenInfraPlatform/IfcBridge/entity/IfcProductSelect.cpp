...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcProductSelect::~IfcProductSelect() {}
		shared_ptr<IfcProductSelect> IfcProductSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcProductSelect::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcProductSelect>();
		}
