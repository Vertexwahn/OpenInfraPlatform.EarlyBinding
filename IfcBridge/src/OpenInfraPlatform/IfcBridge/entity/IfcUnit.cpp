...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcUnit::IfcUnit() {}
		IfcUnit::~IfcUnit() {}
		shared_ptr<IfcUnit> IfcUnit::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcUnit::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcUnit>();
		}
