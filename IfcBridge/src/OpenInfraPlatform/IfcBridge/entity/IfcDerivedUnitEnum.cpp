...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcDerivedUnitEnum::getStepParameter( std::stringstream& stream, bool is_select_type ) const
		{
			...
			else if( m_enum == ENUM_TEMPERATURERATEOFCHANGE )
			{
				stream << ".TEMPERATURERATEOFCHANGE.";
			}
		...
		shared_ptr<IfcDerivedUnitEnum> IfcDerivedUnitEnum::readStepData( std::string& arg )
		{
			...
			else if( _stricmp( arg.c_str(), ".TEMPERATURERATEOFCHANGE." ) == 0 )
			{
				type_object->m_enum = IfcDerivedUnitEnum::ENUM_TEMPERATURERATEOFCHANGE;
			}
