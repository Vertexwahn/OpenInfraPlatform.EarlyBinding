...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcSubContractResource::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcSubContractResource::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<15 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcSubContractResource, expecting 15, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
			if( num_args>15 ){ std::cout << "Wrong parameter count for entity IfcSubContractResource, expecting 15, having " << num_args << ". Object id: " << getId() << std::endl; }
			#endif
		...
		void IfcSubContractResource::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
