...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcPort::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcPort::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<7 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPort, expecting 7, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcPort::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
