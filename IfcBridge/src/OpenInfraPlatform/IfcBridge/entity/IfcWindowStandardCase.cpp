...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcWindowStandardCase::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
			shared_ptr<IfcWindowStandardCase> other = dynamic_pointer_cast<IfcWindowStandardCase>(other_entity);
			if( !other) { return; }
		...
		void IfcWindowStandardCase::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<12 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcWindowStandardCase, expecting 12, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
			if( num_args>12 ){ std::cout << "Wrong parameter count for entity IfcWindowStandardCase, expecting 12, having " << num_args << ". Object id: " << getId() << std::endl; }
			#endif
		...
		void IfcWindowStandardCase::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
