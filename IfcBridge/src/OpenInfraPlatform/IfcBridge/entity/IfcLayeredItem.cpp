...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcLayeredItem::~IfcLayeredItem() {}
		shared_ptr<IfcLayeredItem> IfcLayeredItem::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcLayeredItem::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcLayeredItem>();
		}
