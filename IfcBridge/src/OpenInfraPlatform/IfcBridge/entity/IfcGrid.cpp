...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcGrid::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcGrid::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcGrid, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcGrid::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcBridgeException( "IfcGrid::setInverseCounterparts: type mismatch" ); }
