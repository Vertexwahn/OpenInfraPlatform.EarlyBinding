...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcConstructionResource::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcConstructionResource::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<14 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcConstructionResource, expecting 14, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
			if( num_args>14 ){ std::cout << "Wrong parameter count for entity IfcConstructionResource, expecting 14, having " << num_args << ". Object id: " << getId() << std::endl; }
			#endif
		...
		void IfcConstructionResource::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
