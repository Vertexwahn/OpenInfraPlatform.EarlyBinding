...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcClassificationReference::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcClassificationReference::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<5 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcClassificationReference, expecting 5, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
			if( num_args>5 ){ std::cout << "Wrong parameter count for entity IfcClassificationReference, expecting 5, having " << num_args << ". Object id: " << getId() << std::endl; }
			#endif
		...
		void IfcClassificationReference::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcBridgeException( "IfcClassificationReference::setInverseCounterparts: type mismatch" ); }
