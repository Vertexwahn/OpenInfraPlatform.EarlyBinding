...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcTimeSeries::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcTimeSeries::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTimeSeries, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcTimeSeries::setInverseCounterparts( shared_ptr<IfcBridgeEntity> )
		{
		}
