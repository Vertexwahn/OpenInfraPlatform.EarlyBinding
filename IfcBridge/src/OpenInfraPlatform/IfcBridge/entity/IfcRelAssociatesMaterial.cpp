...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcRelAssociatesMaterial::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcRelAssociatesMaterial::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelAssociatesMaterial, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelAssociatesMaterial::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcBridgeException( "IfcRelAssociatesMaterial::setInverseCounterparts: type mismatch" ); }
