...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcValue::IfcValue() {}
		IfcValue::~IfcValue() {}
		shared_ptr<IfcValue> IfcValue::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				if( keyword.compare("IFCDERIVEDMEASUREVALUE")== 0 )
				{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcValue::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcValue>();
		}
