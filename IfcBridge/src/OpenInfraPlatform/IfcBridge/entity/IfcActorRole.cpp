...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcActorRole::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcActorRole::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<3 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcActorRole, expecting 3, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcActorRole::setInverseCounterparts( shared_ptr<IfcBridgeEntity> )
		{
		}
