...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcTextFontSelect::~IfcTextFontSelect() {}
		shared_ptr<IfcTextFontSelect> IfcTextFontSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcTextFontSelect::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcTextFontSelect>();
		}
