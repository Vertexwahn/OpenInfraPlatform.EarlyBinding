...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcColour::IfcColour() {}
		IfcColour::~IfcColour() {}
		shared_ptr<IfcColour> IfcColour::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcColour::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcColour>();
		}
