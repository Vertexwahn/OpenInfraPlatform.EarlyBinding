...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		IfcProcessSelect::~IfcProcessSelect() {}
		shared_ptr<IfcProcessSelect> IfcProcessSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC2X4::IfcProcessSelect::readStepData" << std::endl;
				throw IfcBridgeException( strs.str() );
			}
			return shared_ptr<IfcProcessSelect>();
		}
