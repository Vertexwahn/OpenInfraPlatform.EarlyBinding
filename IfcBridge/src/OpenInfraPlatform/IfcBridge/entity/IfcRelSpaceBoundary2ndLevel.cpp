...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcRelSpaceBoundary2ndLevel::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcRelSpaceBoundary2ndLevel::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelSpaceBoundary2ndLevel, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelSpaceBoundary2ndLevel::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcBridgeException( "IfcRelSpaceBoundary2ndLevel::setInverseCounterparts: type mismatch" ); }
