...
namespace OpenInfraPlatform
{
	namespace IfcBridge
	{
		...
		void IfcCrewResourceType::setEntity( shared_ptr<IfcBridgeEntity> other_entity )
		{
		...
		void IfcCrewResourceType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcBridgeEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<15 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcCrewResourceType, expecting 15, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcBridgeException( strserr.str().c_str() ); }
			#ifdef _DEBUG
			if( num_args>15 ){ std::cout << "Wrong parameter count for entity IfcCrewResourceType, expecting 15, having " << num_args << ". Object id: " << getId() << std::endl; }
			#endif
			...
			m_PredefinedType = IfcCrewResourceTypeEnum::readStepData( args[14] );
		}
		void IfcCrewResourceType::setInverseCounterparts( shared_ptr<IfcBridgeEntity> ptr_self_entity )
		{
