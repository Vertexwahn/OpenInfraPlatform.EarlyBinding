...
#include "IfcBridgeEntitiesMap.h"
...
namespace OpenInfraPlatform
{
    namespace IfcBridge
    {
        static std::tr1::unordered_map<std::string,IfcBridgeEntityEnum> map_string2entity_enum(initializers_IfcBridge_entity, initializers_IfcBridge_entity + sizeof(initializers_IfcBridge_entity)/sizeof(initializers_IfcBridge_entity[0]));
        void applyBackwardCompatibility( IfcBridgeModel::IfcVersion backward_version, IfcBridgeEntityEnum type_enum, std::vector<std::string>& args );
        void applyBackwardCompatibility( std::string& keyword, std::string& step_line );
        IfcBridgeEntity* createIfcBridgeEntity( const IfcBridgeEntityEnum entity_enum );

        void readStepLine( const std::string& line, shared_ptr<IfcBridgeEntity>& entity )
        {
            ...
            if( keyword.size() > 0 )
            {
                //std::map<std::string,IfcBridgeEntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                std::tr1::unordered_map<std::string,IfcBridgeEntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                ...
                {
                    IfcBridgeEntityEnum entity_enum = it_entity_enum->second;

                    shared_ptr<IfcBridgeEntity> obj( createIfcBridgeEntity( entity_enum ) );
                    if( obj )
                    {
                        obj->setId( id );
                        entity = obj;
        ...
        void IfcStepReader::readStreamHeader( std::string& read_in, shared_ptr<IfcBridgeModel> model )
        {
            ...
            if( file_schema_begin != std::string::npos )
            {
                ...
                if( file_schema_end != std::string::npos )
                {
                    ...
                    {
                        m_ifc_version = IfcBridgeModel::IFC2X2;
                    }
                    else if( m_file_schema.substr(0,6).compare("IFC2X3") == 0 )
                    {
                        m_ifc_version = IfcBridgeModel::IFC2X3;
                    }
                    else if( m_file_schema.substr(0,6).compare("IFC2X4") == 0 )
                    {
                        m_ifc_version = IfcBridgeModel::IFC2X4;
                    }
                    else if( m_file_schema.substr(0,5).compare("IFC2X") == 0 )
                    {
                        m_ifc_version = IfcBridgeModel::IFC2X;
                    }
					else if (m_file_schema.compare("IFCBRIDGE") == 0)
					{
						m_ifc_version = IfcBridgeModel::IFCBRIDGE;
					}
                    else if( m_file_schema.compare("IFC4") == 0 )
                    {
                        m_ifc_version = IfcBridgeModel::IFC4;
                    }
                    ...
                    {
                        m_ifc_version = IfcBridgeModel::UNKNOWN;
                    }
                }
            }
        }
        ...
        void IfcStepReader::readStepLines( const std::vector<std::string>& step_lines, std::vector<shared_ptr<IfcBridgeEntity> >& target_entity_vec )
        {
...
#ifdef IfcBridge_OPENMP
            std::vector<shared_ptr<IfcBridgeEntity> >* target_vec_ptr = &target_entity_vec;
            ...
            #pragma omp parallel firstprivate(num_lines) shared(target_vec_ptr,unkown_entities_ptr)
            {
                ...
                for( int i=0; i<num_lines; ++i )
                {
                    ...
                    try
                    {
                        shared_ptr<IfcBridgeEntity> entity;
        ...
        void IfcStepReader::readEntityArguments( const std::vector<shared_ptr<IfcBridgeEntity> >& vec_entities, const std::map<int,shared_ptr<IfcBridgeEntity> > map_entities )
        {
            ...
            const std::map<int,shared_ptr<IfcBridgeEntity> >* map_ptr = &map_entities;
            const std::vector<shared_ptr<IfcBridgeEntity> >* vec_entities_ptr = &vec_entities;

            #pragma omp parallel firstprivate(num_objects) shared(map_ptr,vec_entities_ptr)
            {
                const std::map<int,shared_ptr<IfcBridgeEntity> > &map_loc = *map_ptr;
                std::stringstream thread_err;

                #pragma omp for schedule(dynamic, 100)
                for( int i=0; i<num_objects; ++i )
                {
                    shared_ptr<IfcBridgeEntity> entity = vec_entities_ptr->at(i);
                    std::string& argument_str = entity->m_arguments;
                    std::vector<std::string> arguments;
                    tokenizeEntityArguments( argument_str, arguments );

                    if( m_ifc_version != IfcBridgeModel::IFC4 )
                    {
                        if( m_ifc_version != IfcBridgeModel::UNDEFINED && m_ifc_version != IfcBridgeModel::UNKNOWN )
                        {
                            IfcBridgeEntityEnum entity_enum = entity->m_entity_enum;
            ...
            std::vector<shared_ptr<IfcBridgeEntity> >::const_iterator it_entity_vec = vec_entities.begin();
            int i=0;
            for( ; it_entity_vec!=vec_entities.end(); ++it_entity_vec )
            {
                shared_ptr<IfcBridgeEntity> entity = (*it_entity_vec);
                std::string& argument_str = entity->m_arguments;
                std::vector<std::string> arguments;
                tokenizeEntityArguments( argument_str, arguments );
...
#endif // IfcBridge_OPENMP

            if( err.tellp() > 0 )
            {
                throw IfcBridgeException( err.str() );
            }
        }

        void IfcStepReader::readStreamData(	std::string& read_in, std::map<int,shared_ptr<IfcBridgeEntity> >& target_map )
        {
            ...
            std::vector<shared_ptr<IfcBridgeEntity> > vec_entities;
            try
            {
        ...
        void applyBackwardCompatibility( IfcBridgeModel::IfcVersion version, IfcBridgeEntityEnum type_enum, std::vector<std::string>& args )
        {
            // TODO: replace this workaround with a systematic backward compatibility, possibly generated from schema diff
            if( version < IfcBridgeModel::IFC2X )
            {
                throw IfcBridgeException( "Unsupported IFC version" );
            }
            if( version <= IfcBridgeModel::IFC2X )
            {
            ...
            if( version <= IfcBridgeModel::IFC2X2 )
            {
                switch( type_enum )
                {
                case IFCCOLOURRGB:
            ...
            if( version <= IfcBridgeModel::IFC2X3 )
            {
                switch( type_enum )
                {
                // B
            ...
            if( version <= IfcBridgeModel::IFC4 )
            {
                switch( type_enum )
                {
                // I
