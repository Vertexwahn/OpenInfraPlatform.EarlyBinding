    ...
    namespace IfcBridge
    {
        ...
        void readTypeOfRealList( const char* str, std::vector<shared_ptr<T> >& vec )
        {
            ...
            while( *ch != '\0' )
            {
                ...
                if( strchr(ch, '$') != NULL )
                {
                    throw IfcBridgeException("unexpected $");
                }
#endif
        ...
        void readTypeOfRealList2D( std::string& str, std::vector<std::vector<shared_ptr<T> > >& vec )
        {
            ...
            if( ch[0] != '(' )
            {
                throw IfcBridgeException( "readTypeOfRealList2D: string does not start with (" );
            }
            unsigned int i=0;
        ...
        template<class T>
        void readEntityReference(  std::string& str, shared_ptr<T>& smart, const std::map<int,shared_ptr<IfcBridgeEntity> >& map_entities )
        {
            if( str.length() == 0)
            {
                std::stringstream err;
            ...
            if( str.at(0) == '#' )
            {
                ...
                std::map<int,shared_ptr<IfcBridgeEntity> >::const_iterator it_entity = map_entities.find( entity_id );
                if( it_entity != map_entities.end() )
                {
                    shared_ptr<IfcBridgeEntity> found_obj = it_entity->second;
            ...
            {
                throw IfcBridgeException( "readEntityReference: unexpected argument\n" );
            }
        }
        ...
        template<class T>
        void readSelectList( std::string& arg_complete, std::vector<shared_ptr<T> >& vec, const std::map<int,shared_ptr<IfcBridgeEntity> >& map_entities )
        {
            //(#287,#291,#295,#299)
            char* pos_opening = NULL;
            char* pos_closing = NULL;
            ...
            std::map<int,shared_ptr<IfcBridgeEntity> >::const_iterator it_entity;
            for( unsigned int i=0; i<list_items.size(); ++i )
            {
                std::string& item = list_items[i];
        ...
        template<class T>
        void readEntityReferenceList( const char* arg_complete, std::vector<shared_ptr<T> >& vec, const std::map<int,shared_ptr<IfcBridgeEntity> >& map_entities )
        {
            //(#287,#291,#295,#299)
            char* pos_opening = NULL;
            char* pos_closing = NULL;
        ...
        template<class T>
        void readEntityReferenceList( const std::string& str, std::vector<shared_ptr<T> >& vec, const std::map<int,shared_ptr<IfcBridgeEntity> >& map_entities )
        {
            //(#287,#291,#295,#299)
            char* ch = (char*)str.c_str();
        ...
        template<class T>
        void readEntityReferenceList2D( std::string& str, std::vector<std::vector<shared_ptr<T> > >& vec, const std::map<int,shared_ptr<IfcBridgeEntity> >& map_entities )
        {
        ...
        template<class T>
        void readEntityReferenceList3D( std::string& str, std::vector<std::vector<std::vector<shared_ptr<T> > > >& vec, const std::map<int,shared_ptr<IfcBridgeEntity> >& map_entities )
        {
