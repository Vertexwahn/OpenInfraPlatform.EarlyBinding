...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcAsset::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcAsset> other = std::dynamic_pointer_cast<IfcAsset>(other_entity);
				if( !other) { return; }
			...
			void IfcAsset::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_Owner) { m_Owner->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_User) { m_User->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
			...
			void IfcAsset::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<14 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcAsset, expecting 14, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcAsset::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
