...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcThermodynamicTemperatureMeasure> IfcThermodynamicTemperatureMeasure::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcThermodynamicTemperatureMeasure>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcThermodynamicTemperatureMeasure>(); }
			std::shared_ptr<IfcThermodynamicTemperatureMeasure> type_object(new IfcThermodynamicTemperatureMeasure() );
