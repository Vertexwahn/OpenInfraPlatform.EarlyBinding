...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcConnectionCurveGeometry::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcConnectionCurveGeometry> other = std::dynamic_pointer_cast<IfcConnectionCurveGeometry>(other_entity);
				if( !other) { return; }
			...
			void IfcConnectionCurveGeometry::getStepLine( std::stringstream& stream ) const
			{
				...
				if( m_CurveOnRelatingElement) { m_CurveOnRelatingElement->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_CurveOnRelatedElement) { m_CurveOnRelatedElement->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcConnectionCurveGeometry::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcConnectionCurveGeometry::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcConnectionCurveGeometry, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcConnectionCurveGeometry::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
