...
#include "IfcStructuralLoadConfiguration.h"
#include "IfcLabel.h"
...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcStructuralLoadConfiguration::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcStructuralLoadConfiguration> other = std::dynamic_pointer_cast<IfcStructuralLoadConfiguration>(other_entity);
				if( !other) { return; }
				m_Name = other->m_Name;
			...
			void IfcStructuralLoadConfiguration::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<3 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcStructuralLoadConfiguration, expecting 3, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcStructuralLoadConfiguration::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
