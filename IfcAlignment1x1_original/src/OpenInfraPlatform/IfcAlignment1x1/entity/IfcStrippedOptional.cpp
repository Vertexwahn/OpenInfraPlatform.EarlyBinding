...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcStrippedOptional> IfcStrippedOptional::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcStrippedOptional>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcStrippedOptional>(); }
			std::shared_ptr<IfcStrippedOptional> type_object(new IfcStrippedOptional() );
