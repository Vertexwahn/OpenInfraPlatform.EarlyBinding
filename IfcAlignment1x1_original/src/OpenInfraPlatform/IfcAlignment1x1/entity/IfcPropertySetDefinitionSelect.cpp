...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		IfcPropertySetDefinitionSelect::~IfcPropertySetDefinitionSelect() {}
		std::shared_ptr<IfcPropertySetDefinitionSelect> IfcPropertySetDefinitionSelect::readStepData(const std::string& arg, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map)
		{
			// Read SELECT TYPE
			if( arg.size() == 0 ) { return std::shared_ptr<IfcPropertySetDefinitionSelect>(); }
			if( arg[0] == '#' )
			{
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IfcAlignment1x1::IfcPropertySetDefinitionSelect::readStepData" << std::endl;
				throw IfcAlignment1x1Exception( strs.str() );
			}
			return shared_ptr<IfcPropertySetDefinitionSelect>();
		}
