...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcSurfaceStyleRendering::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcSurfaceStyleRendering> other = std::dynamic_pointer_cast<IfcSurfaceStyleRendering>(other_entity);
				if( !other) { return; }
			...
			void IfcSurfaceStyleRendering::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_DiffuseColour) { m_DiffuseColour->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_TransmissionColour) { m_TransmissionColour->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_DiffuseTransmissionColour) { m_DiffuseTransmissionColour->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_ReflectionColour) { m_ReflectionColour->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_SpecularColour) { m_SpecularColour->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_SpecularHighlight) { m_SpecularHighlight->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
			...
			void IfcSurfaceStyleRendering::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcSurfaceStyleRendering, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcSurfaceStyleRendering::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
