...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcConnectionVolumeGeometry::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcConnectionVolumeGeometry> other = std::dynamic_pointer_cast<IfcConnectionVolumeGeometry>(other_entity);
				if( !other) { return; }
			...
			void IfcConnectionVolumeGeometry::getStepLine( std::stringstream& stream ) const
			{
				...
				if( m_VolumeOnRelatingElement) { m_VolumeOnRelatingElement->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_VolumeOnRelatedElement) { m_VolumeOnRelatedElement->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcConnectionVolumeGeometry::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcConnectionVolumeGeometry::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcConnectionVolumeGeometry, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcConnectionVolumeGeometry::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
