...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcThermalExpansionCoefficientMeasure> IfcThermalExpansionCoefficientMeasure::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcThermalExpansionCoefficientMeasure>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcThermalExpansionCoefficientMeasure>(); }
			std::shared_ptr<IfcThermalExpansionCoefficientMeasure> type_object(new IfcThermalExpansionCoefficientMeasure() );
