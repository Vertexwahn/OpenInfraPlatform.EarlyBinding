...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcPropertyTableValue::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcPropertyTableValue> other = std::dynamic_pointer_cast<IfcPropertyTableValue>(other_entity);
				if( !other) { return; }
				m_Name = other->m_Name;
			...
			void IfcPropertyTableValue::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				writeTypeList( stream, m_DefiningValues );
				stream << ",";
				writeTypeList( stream, m_DefinedValues );
				stream << ",";
				if( m_Expression) { m_Expression->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_DefiningUnit) { m_DefiningUnit->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_DefinedUnit) { m_DefinedUnit->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
			...
			void IfcPropertyTableValue::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPropertyTableValue, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcPropertyTableValue::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
