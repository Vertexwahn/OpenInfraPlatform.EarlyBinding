...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcDimensionCount> IfcDimensionCount::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcDimensionCount>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcDimensionCount>(); }
			std::shared_ptr<IfcDimensionCount> type_object(new IfcDimensionCount() );
