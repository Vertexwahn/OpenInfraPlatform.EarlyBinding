...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcTransportElementType::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcTransportElementType> other = std::dynamic_pointer_cast<IfcTransportElementType>(other_entity);
				if( !other) { return; }
			...
			void IfcTransportElementType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTransportElementType, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
				if( num_args<10 ){ std::cout << "Wrong parameter count for entity IfcTransportElementType, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; }
				#endif
				...
				m_PredefinedType = IfcTransportElementTypeEnum::readStepData( args[9] );
			}
			void IfcTransportElementType::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
