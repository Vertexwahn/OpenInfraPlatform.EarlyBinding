...
#include "IfcStructuralCurveReaction.h"
...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcStructuralCurveReaction::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcStructuralCurveReaction> other = std::dynamic_pointer_cast<IfcStructuralCurveReaction>(other_entity);
				if( !other) { return; }
			...
			void IfcStructuralCurveReaction::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcStructuralCurveReaction, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
				...
				m_PredefinedType = IfcStructuralCurveActivityTypeEnum::readStepData( args[9] );
			}
			void IfcStructuralCurveReaction::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
