...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcShapeAspect::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcShapeAspect> other = std::dynamic_pointer_cast<IfcShapeAspect>(other_entity);
				if( !other) { return; }
			...
			void IfcShapeAspect::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_PartOfProductDefinitionShape) { m_PartOfProductDefinitionShape->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcShapeAspect::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcShapeAspect::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<5 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcShapeAspect, expecting 5, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcShapeAspect::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
			}
