...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcRelAssociatesLibrary::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcRelAssociatesLibrary> other = std::dynamic_pointer_cast<IfcRelAssociatesLibrary>(other_entity);
				if( !other) { return; }
			...
			void IfcRelAssociatesLibrary::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_RelatingLibrary) { m_RelatingLibrary->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcRelAssociatesLibrary::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcRelAssociatesLibrary::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelAssociatesLibrary, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcRelAssociatesLibrary::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
