...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcCrewResourceType::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcCrewResourceType> other = std::dynamic_pointer_cast<IfcCrewResourceType>(other_entity);
				if( !other) { return; }
			...
			void IfcCrewResourceType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<12 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcCrewResourceType, expecting 12, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
				if( num_args<12 ){ std::cout << "Wrong parameter count for entity IfcCrewResourceType, expecting 12, having " << num_args << ". Object id: " << getId() << std::endl; }
				#endif
				...
				m_PredefinedType = IfcCrewResourceTypeEnum::readStepData( args[11] );
			}
			void IfcCrewResourceType::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
				IfcConstructionResourceType::setInverseCounterparts(ptr_self_entity);
			}
			void IfcCrewResourceType::unlinkSelf()
			{
