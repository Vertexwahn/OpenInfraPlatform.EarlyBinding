...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcRepresentationMap::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcRepresentationMap> other = std::dynamic_pointer_cast<IfcRepresentationMap>(other_entity);
				if( !other) { return; }
			...
			void IfcRepresentationMap::getStepLine( std::stringstream& stream ) const
			{
				...
				if( m_MappingOrigin) { m_MappingOrigin->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
			...
			void IfcRepresentationMap::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRepresentationMap, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcRepresentationMap::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
			}
