...
#include "IfcReinforcingBarType.h"
...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcReinforcingBarType::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcReinforcingBarType> other = std::dynamic_pointer_cast<IfcReinforcingBarType>(other_entity);
				if( !other) { return; }
			...
			void IfcReinforcingBarType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<16 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcReinforcingBarType, expecting 16, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcReinforcingBarType::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
