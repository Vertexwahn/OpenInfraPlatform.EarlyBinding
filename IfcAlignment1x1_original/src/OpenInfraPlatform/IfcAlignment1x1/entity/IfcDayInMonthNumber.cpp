...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcDayInMonthNumber> IfcDayInMonthNumber::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcDayInMonthNumber>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcDayInMonthNumber>(); }
			std::shared_ptr<IfcDayInMonthNumber> type_object(new IfcDayInMonthNumber() );
