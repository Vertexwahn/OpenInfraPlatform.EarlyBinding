...
#include "IfcReinforcingMeshType.h"
...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcReinforcingMeshType::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcReinforcingMeshType> other = std::dynamic_pointer_cast<IfcReinforcingMeshType>(other_entity);
				if( !other) { return; }
			...
			void IfcReinforcingMeshType::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				writeTypeList( stream, m_BendingParameters );
				stream << ");";
			}
			void IfcReinforcingMeshType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcReinforcingMeshType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<20 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcReinforcingMeshType, expecting 20, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcReinforcingMeshType::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
