...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcRelAssignsToProcess::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcRelAssignsToProcess> other = std::dynamic_pointer_cast<IfcRelAssignsToProcess>(other_entity);
				if( !other) { return; }
			...
			void IfcRelAssignsToProcess::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_RelatingProcess) { m_RelatingProcess->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
			...
			void IfcRelAssignsToProcess::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelAssignsToProcess, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcRelAssignsToProcess::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
