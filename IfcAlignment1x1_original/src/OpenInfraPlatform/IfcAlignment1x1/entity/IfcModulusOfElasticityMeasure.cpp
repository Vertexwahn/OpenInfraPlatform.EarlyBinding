...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcModulusOfElasticityMeasure> IfcModulusOfElasticityMeasure::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcModulusOfElasticityMeasure>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcModulusOfElasticityMeasure>(); }
			std::shared_ptr<IfcModulusOfElasticityMeasure> type_object(new IfcModulusOfElasticityMeasure() );
