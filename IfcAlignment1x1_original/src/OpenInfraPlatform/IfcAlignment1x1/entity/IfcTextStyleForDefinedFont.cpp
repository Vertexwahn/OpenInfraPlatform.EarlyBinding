...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcTextStyleForDefinedFont::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcTextStyleForDefinedFont> other = std::dynamic_pointer_cast<IfcTextStyleForDefinedFont>(other_entity);
				if( !other) { return; }
			...
			void IfcTextStyleForDefinedFont::getStepLine( std::stringstream& stream ) const
			{
				...
				if( m_Colour) { m_Colour->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_BackgroundColour) { m_BackgroundColour->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcTextStyleForDefinedFont::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcTextStyleForDefinedFont::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTextStyleForDefinedFont, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcTextStyleForDefinedFont::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
