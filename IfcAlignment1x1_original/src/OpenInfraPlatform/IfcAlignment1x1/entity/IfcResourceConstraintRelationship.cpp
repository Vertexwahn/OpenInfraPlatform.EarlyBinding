...
#include "IfcResourceConstraintRelationship.h"
#include "IfcLabel.h"
#include "IfcText.h"
...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcResourceConstraintRelationship::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcResourceConstraintRelationship> other = std::dynamic_pointer_cast<IfcResourceConstraintRelationship>(other_entity);
				if( !other) { return; }
				m_Name = other->m_Name;
			...
			void IfcResourceConstraintRelationship::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				writeTypeList( stream, m_RelatedResourceObjects );
				stream << ");";
			}
			void IfcResourceConstraintRelationship::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcResourceConstraintRelationship::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<4 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcResourceConstraintRelationship, expecting 4, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcResourceConstraintRelationship::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
