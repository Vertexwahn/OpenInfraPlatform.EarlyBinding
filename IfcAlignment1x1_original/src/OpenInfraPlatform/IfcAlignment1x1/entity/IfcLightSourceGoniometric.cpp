...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcLightSourceGoniometric::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcLightSourceGoniometric> other = std::dynamic_pointer_cast<IfcLightSourceGoniometric>(other_entity);
				if( !other) { return; }
				m_Name = other->m_Name;
			...
			void IfcLightSourceGoniometric::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_LightDistributionDataSource) { m_LightDistributionDataSource->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcLightSourceGoniometric::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcLightSourceGoniometric::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcLightSourceGoniometric, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcLightSourceGoniometric::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
