...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		IfcDefinitionSelect::~IfcDefinitionSelect() {}
		std::shared_ptr<IfcDefinitionSelect> IfcDefinitionSelect::readStepData(const std::string& arg, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map)
		{
			// Read SELECT TYPE
			if( arg.size() == 0 ) { return std::shared_ptr<IfcDefinitionSelect>(); }
			if( arg[0] == '#' )
			{
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IfcAlignment1x1::IfcDefinitionSelect::readStepData" << std::endl;
				throw IfcAlignment1x1Exception( strs.str() );
			}
			return shared_ptr<IfcDefinitionSelect>();
		}
