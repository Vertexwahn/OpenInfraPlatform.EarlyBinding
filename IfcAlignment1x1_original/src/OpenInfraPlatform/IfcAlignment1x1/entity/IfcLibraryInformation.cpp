...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcLibraryInformation::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcLibraryInformation> other = std::dynamic_pointer_cast<IfcLibraryInformation>(other_entity);
				if( !other) { return; }
				m_Name = other->m_Name;
			...
			void IfcLibraryInformation::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_Publisher) { m_Publisher->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
			...
			void IfcLibraryInformation::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcLibraryInformation, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcLibraryInformation::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
