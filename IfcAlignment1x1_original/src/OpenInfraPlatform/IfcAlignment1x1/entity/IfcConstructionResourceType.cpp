...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcConstructionResourceType::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcConstructionResourceType> other = std::dynamic_pointer_cast<IfcConstructionResourceType>(other_entity);
				if( !other) { return; }
			...
			void IfcConstructionResourceType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcConstructionResourceType, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
				if( num_args<11 ){ std::cout << "Wrong parameter count for entity IfcConstructionResourceType, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; }
				#endif
			...
			void IfcConstructionResourceType::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
				IfcTypeResource::setInverseCounterparts(ptr_self_entity);
			}
			void IfcConstructionResourceType::unlinkSelf()
			{
				IfcTypeResource::unlinkSelf();
			}
	}
}
