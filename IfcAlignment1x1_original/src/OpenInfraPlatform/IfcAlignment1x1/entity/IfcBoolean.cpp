...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcBoolean> IfcBoolean::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcBoolean>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcBoolean>(); }
			std::shared_ptr<IfcBoolean> type_object(new IfcBoolean() );
