...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcFillAreaStyleHatching::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcFillAreaStyleHatching> other = std::dynamic_pointer_cast<IfcFillAreaStyleHatching>(other_entity);
				if( !other) { return; }
			...
			void IfcFillAreaStyleHatching::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_StartOfNextHatchLine) { m_StartOfNextHatchLine->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
			...
			void IfcFillAreaStyleHatching::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<5 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcFillAreaStyleHatching, expecting 5, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcFillAreaStyleHatching::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
