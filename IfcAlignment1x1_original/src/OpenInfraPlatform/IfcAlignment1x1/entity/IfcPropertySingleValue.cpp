...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcPropertySingleValue::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcPropertySingleValue> other = std::dynamic_pointer_cast<IfcPropertySingleValue>(other_entity);
				if( !other) { return; }
				m_Name = other->m_Name;
			...
			void IfcPropertySingleValue::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_NominalValue) { m_NominalValue->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
			...
			void IfcPropertySingleValue::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<4 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPropertySingleValue, expecting 4, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcPropertySingleValue::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
