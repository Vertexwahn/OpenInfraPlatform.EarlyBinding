...
#include "model/UnknownEntityException.h"
#include "IfcAlignment1x1EntitiesMap.h"
#include "reader/ReaderUtil.h"
#include "reader/IfcStepReader.h"

namespace OpenInfraPlatform
{
    namespace IfcAlignment1x1
    {
        static std::unordered_map<std::string,IfcAlignment1x1EntityEnum> map_string2entity_enum(initializers_IfcAlignment1x1_entity, initializers_IfcAlignment1x1_entity + sizeof(initializers_IfcAlignment1x1_entity)/sizeof(initializers_IfcAlignment1x1_entity[0]));
        void applyBackwardCompatibility( IfcAlignment1x1Model::IfcVersion backward_version, IfcAlignment1x1EntityEnum type_enum, std::vector<std::string>& args );
        void applyBackwardCompatibility( std::string& keyword, std::string& step_line );
        IfcAlignment1x1Entity* createIfcAlignment1x1Entity( const IfcAlignment1x1EntityEnum entity_enum );

        void readStepLine( const std::string& line, shared_ptr<IfcAlignment1x1Entity>& entity )
        {
            ...
            if( keyword.size() > 0 )
            {
                //std::map<std::string,IfcAlignment1x1EntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                std::unordered_map<std::string,IfcAlignment1x1EntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                ...
                {
                    IfcAlignment1x1EntityEnum entity_enum = it_entity_enum->second;

                    shared_ptr<IfcAlignment1x1Entity> obj( createIfcAlignment1x1Entity( entity_enum ) );
                    if( obj )
                    {
                        obj->setId( id );
                        entity = obj;
        ...
        void IfcStepReader::readStreamHeader( std::string& read_in, shared_ptr<IfcAlignment1x1Model> model )
        {
            m_ifc_version = IfcAlignment1x1Model::UNDEFINED;
            m_file_schema = "";
            ...
            if( file_schema_begin != std::string::npos )
            {
                ...
                if( file_schema_end != std::string::npos )
                {
					...
					{
						// hack: assume IfcAlignment1x1
						m_file_schema = "IfcAlignment1x1";
					}

                    model->setFileSchema( m_file_schema );
                    if( m_file_schema.substr(0,6).compare("IFC2X2") == 0 )
                    {
                        m_ifc_version = IfcAlignment1x1Model::IFC2X2;
                    }
                    else if( m_file_schema.substr(0,6).compare("IFC2X3") == 0 )
                    {
                        m_ifc_version = IfcAlignment1x1Model::IFC2X3;
                    }
                    else if( m_file_schema.substr(0,6).compare("IFC2X4") == 0 )
                    {
                        m_ifc_version = IfcAlignment1x1Model::IFC2X4;
                    }
                    else if( m_file_schema.substr(0,5).compare("IFC2X") == 0 )
                    {
                        m_ifc_version = IfcAlignment1x1Model::IFC2X;
                    }
                    else if( m_file_schema.compare("IfcAlignment1x1") == 0 )
                    {
                        m_ifc_version = IfcAlignment1x1Model::IfcAlignment1x1;
                    }
                    else if( m_file_schema.compare("IfcAlignment1x1RC4") == 0 )
                    {
                        m_ifc_version = IfcAlignment1x1Model::IfcAlignment1x1;
                    }
					else if( m_file_schema.compare("IFC4X1") == 0 )
                    {
                        m_ifc_version = IfcAlignment1x1Model::Ifc4x1;
                    }
                    else
                    {
                        m_ifc_version = IfcAlignment1x1Model::UNKNOWN;
                    }
                }
            }
        }
        ...
        void IfcStepReader::readStepLines( const std::vector<std::string>& step_lines, std::vector<shared_ptr<IfcAlignment1x1Entity> >& target_entity_vec )
        {
...
#ifdef IfcAlignment1x1_OPENMP
            std::vector<shared_ptr<IfcAlignment1x1Entity> >* target_vec_ptr = &target_entity_vec;
            ...
            #pragma omp parallel firstprivate(num_lines) shared(target_vec_ptr,unkown_entities_ptr)
            {
                ...
                for( int i=0; i<num_lines; ++i )
                {
                    ...
                    try
                    {
                        shared_ptr<IfcAlignment1x1Entity> entity;
        ...
        void IfcStepReader::readEntityArguments( const std::vector<shared_ptr<IfcAlignment1x1Entity> >& vec_entities, const std::map<int,shared_ptr<IfcAlignment1x1Entity> > map_entities )
        {
            ...
            const std::map<int,shared_ptr<IfcAlignment1x1Entity> >* map_ptr = &map_entities;
            const std::vector<shared_ptr<IfcAlignment1x1Entity> >* vec_entities_ptr = &vec_entities;

            #pragma omp parallel firstprivate(num_objects) shared(map_ptr,vec_entities_ptr)
            {
                const std::map<int,shared_ptr<IfcAlignment1x1Entity> > &map_loc = *map_ptr;
                std::stringstream thread_err;

                #pragma omp for schedule(dynamic, 100)
                for( int i=0; i<num_objects; ++i )
                {
                    shared_ptr<IfcAlignment1x1Entity> entity = vec_entities_ptr->at(i);
                    std::string& argument_str = entity->m_arguments;
                    std::vector<std::string> arguments;
                    tokenizeEntityArguments( argument_str, arguments );

                    if( m_ifc_version != IfcAlignment1x1Model::IfcAlignment1x1 )
                    {
                        if( m_ifc_version != IfcAlignment1x1Model::UNDEFINED && m_ifc_version != IfcAlignment1x1Model::UNKNOWN )
                        {
                            IfcAlignment1x1EntityEnum entity_enum = entity->m_entity_enum;
            ...
            std::vector<shared_ptr<IfcAlignment1x1Entity> >::const_iterator it_entity_vec = vec_entities.begin();
            int i=0;
            for( ; it_entity_vec!=vec_entities.end(); ++it_entity_vec )
            {
                shared_ptr<IfcAlignment1x1Entity> entity = (*it_entity_vec);
        ...
        void IfcStepReader::readStreamData(	std::string& read_in, std::map<int,shared_ptr<IfcAlignment1x1Entity> >& target_map )
        {
            ...
            std::vector<shared_ptr<IfcAlignment1x1Entity> > vec_entities;
            try
            {
        ...
        void applyBackwardCompatibility( IfcAlignment1x1Model::IfcVersion version, IfcAlignment1x1EntityEnum type_enum, std::vector<std::string>& args )
        {
            // TODO: replace this workaround with a systematic backward compatibility, possibly generated from schema diff
            if( version < IfcAlignment1x1Model::IFC2X )
            {
                throw IfcAlignment1x1Exception( "Unsupported IFC version" );
            }
            if( version <= IfcAlignment1x1Model::IFC2X )
            {
            ...
            if( version <= IfcAlignment1x1Model::IFC2X2 )
            {
                switch( type_enum )
                {
                case IFCCOLOURRGB:
            ...
            if( version <= IfcAlignment1x1Model::IFC2X3 )
            {
                switch( type_enum )
                {
                // B
...
//                     break;
            ...
            if( version <= IfcAlignment1x1Model::IfcAlignment1x1 )
            {
                switch( type_enum )
                {
                // I
