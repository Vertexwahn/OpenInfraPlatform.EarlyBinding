...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcConstructionResourceType::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
			shared_ptr<IfcConstructionResourceType> other = dynamic_pointer_cast<IfcConstructionResourceType>(other_entity);
			if( !other) { return; }
		...
		void IfcConstructionResourceType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcConstructionResourceType, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcConstructionResourceType::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
