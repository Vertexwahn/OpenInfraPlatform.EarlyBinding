...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		IfcCsgSelect::~IfcCsgSelect() {}
		shared_ptr<IfcCsgSelect> IfcCsgSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC4::IfcCsgSelect::readStepData" << std::endl;
				throw IfcAlignmentP6Exception( strs.str() );
			}
			return shared_ptr<IfcCsgSelect>();
		}
