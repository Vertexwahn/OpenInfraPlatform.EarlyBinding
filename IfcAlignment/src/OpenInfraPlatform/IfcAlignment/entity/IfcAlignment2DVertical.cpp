...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcAlignment2DVertical::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcAlignment2DVertical::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<1 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcAlignment2DVertical, expecting 1, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcAlignment2DVertical::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
			shared_ptr<IfcAlignment2DVertical> ptr_self = dynamic_pointer_cast<IfcAlignment2DVertical>( ptr_self_entity );
			if( !ptr_self ) { throw IfcAlignmentP6Exception( "IfcAlignment2DVertical::setInverseCounterparts: type mismatch" ); }
