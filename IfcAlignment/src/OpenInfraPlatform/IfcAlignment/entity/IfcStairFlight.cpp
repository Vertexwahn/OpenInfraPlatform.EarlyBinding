...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcStairFlight::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcStairFlight::getStepLine( std::stringstream& stream ) const
		{
			...
			stream << ",";
			if( m_NumberOfRisers == m_NumberOfRisers ){ stream << m_NumberOfRisers; }
			else { stream << "$"; }
			stream << ",";
		...
		void IfcStairFlight::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<13 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcStairFlight, expecting 13, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
			...
			readIntValue( args[8], m_NumberOfRisers );
		...
		void IfcStairFlight::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
