...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcVirtualGridIntersection::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcVirtualGridIntersection::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcVirtualGridIntersection, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcVirtualGridIntersection::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
			shared_ptr<IfcVirtualGridIntersection> ptr_self = dynamic_pointer_cast<IfcVirtualGridIntersection>( ptr_self_entity );
			if( !ptr_self ) { throw IfcAlignmentP6Exception( "IfcVirtualGridIntersection::setInverseCounterparts: type mismatch" ); }
