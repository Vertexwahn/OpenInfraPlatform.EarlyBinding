...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcRelReferencedInSpatialStructure::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcRelReferencedInSpatialStructure::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelReferencedInSpatialStructure, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelReferencedInSpatialStructure::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcAlignmentP6Exception( "IfcRelReferencedInSpatialStructure::setInverseCounterparts: type mismatch" ); }
