...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcRelContainedInSpatialStructure::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcRelContainedInSpatialStructure::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelContainedInSpatialStructure, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelContainedInSpatialStructure::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcAlignmentP6Exception( "IfcRelContainedInSpatialStructure::setInverseCounterparts: type mismatch" ); }
			for( int i=0; i<m_RelatedElements.size(); ++i )
			{
				shared_ptr<IfcAlignment>  RelatedElements_IfcAlignment = dynamic_pointer_cast<IfcAlignment>( m_RelatedElements[i] );
				if( RelatedElements_IfcAlignment )
				{
					RelatedElements_IfcAlignment->m_ContainedInStructure_inverse.push_back( ptr_self );
				}
		...
		void IfcRelContainedInSpatialStructure::unlinkSelf()
		{
			...
			for( int i=0; i<m_RelatedElements.size(); ++i )
			{
				...
				if( RelatedElements_IfcAlignment )
				{
					std::vector<weak_ptr<IfcRelContainedInSpatialStructure> >& ContainedInStructure_inverse = RelatedElements_IfcAlignment->m_ContainedInStructure_inverse;
