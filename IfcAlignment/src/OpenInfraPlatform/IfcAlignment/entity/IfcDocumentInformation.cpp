...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcDocumentInformation::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcDocumentInformation::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<17 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcDocumentInformation, expecting 17, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcDocumentInformation::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
