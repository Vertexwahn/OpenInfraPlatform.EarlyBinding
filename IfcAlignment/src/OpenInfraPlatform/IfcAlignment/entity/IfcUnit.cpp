...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		IfcUnit::IfcUnit() {}
		IfcUnit::~IfcUnit() {}
		shared_ptr<IfcUnit> IfcUnit::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			// Read SELECT TYPE
			...
			else
			{
				...
				std::stringstream strs;
				strs << "unhandled inline argument: " << arg << " in function IFC4::IfcUnit::readStepData" << std::endl;
				throw IfcAlignmentP6Exception( strs.str() );
			}
			return shared_ptr<IfcUnit>();
		}
