...
#include "include/IfcAlignment2DVerSegParabolicArc.h"
...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcAlignment2DVerSegParabolicArc::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
			shared_ptr<IfcAlignment2DVerSegParabolicArc> other = dynamic_pointer_cast<IfcAlignment2DVerSegParabolicArc>(other_entity);
			if( !other) { return; }
		...
		void IfcAlignment2DVerSegParabolicArc::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcAlignment2DVerSegParabolicArc, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcAlignment2DVerSegParabolicArc::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
