...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcStyledItem::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcStyledItem::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<3 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcStyledItem, expecting 3, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcStyledItem::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcAlignmentP6Exception( "IfcStyledItem::setInverseCounterparts: type mismatch" ); }
			if( m_Item )
			{
