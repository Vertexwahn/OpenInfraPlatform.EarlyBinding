...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcComplexPropertyTemplate::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcComplexPropertyTemplate::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<7 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcComplexPropertyTemplate, expecting 7, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcComplexPropertyTemplate::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
			...
			if( !ptr_self ) { throw IfcAlignmentP6Exception( "IfcComplexPropertyTemplate::setInverseCounterparts: type mismatch" ); }
