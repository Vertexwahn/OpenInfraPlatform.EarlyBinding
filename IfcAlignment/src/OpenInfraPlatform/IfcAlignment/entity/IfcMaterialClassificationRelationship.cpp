...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcMaterialClassificationRelationship::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcMaterialClassificationRelationship::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcMaterialClassificationRelationship, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcMaterialClassificationRelationship::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> )
		{
		}
