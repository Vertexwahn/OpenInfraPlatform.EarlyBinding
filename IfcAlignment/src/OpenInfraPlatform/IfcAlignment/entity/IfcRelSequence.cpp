...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcRelSequence::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
		...
		void IfcRelSequence::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
			IfcRelConnects::setInverseCounterparts( ptr_self_entity );
			shared_ptr<IfcRelSequence> ptr_self = dynamic_pointer_cast<IfcRelSequence>( ptr_self_entity );
			if( !ptr_self ) { throw IfcAlignmentP6Exception( "IfcRelSequence::setInverseCounterparts: type mismatch" ); }
			if( m_RelatedProcess )
			{
