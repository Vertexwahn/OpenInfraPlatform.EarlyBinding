...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcDistributionFlowElement::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
			shared_ptr<IfcDistributionFlowElement> other = dynamic_pointer_cast<IfcDistributionFlowElement>(other_entity);
			if( !other) { return; }
		...
		void IfcDistributionFlowElement::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcDistributionFlowElement, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcDistributionFlowElement::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
