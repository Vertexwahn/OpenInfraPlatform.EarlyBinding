...
namespace OpenInfraPlatform
{
	namespace IfcAlignment
	{
		...
		void IfcElectricMotorType::setEntity( shared_ptr<IfcAlignmentP6Entity> other_entity )
		{
			shared_ptr<IfcElectricMotorType> other = dynamic_pointer_cast<IfcElectricMotorType>(other_entity);
			if( !other) { return; }
		...
		void IfcElectricMotorType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignmentP6Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcElectricMotorType, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignmentP6Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcElectricMotorType::setInverseCounterparts( shared_ptr<IfcAlignmentP6Entity> ptr_self_entity )
		{
