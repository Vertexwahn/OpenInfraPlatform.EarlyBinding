...
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcPerson.h"
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcOrganization.h"
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcPersonAndOrganization.h"
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcApplication.h"
...
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcAxis2Placement3D.h"
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcOwnerHistory.h"
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcDimensionalExponents.h"
...
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcGeometricRepresentationContext.h"
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcProduct.h"
#include "OpenInfraPlatform/IfcAlignment/entity/include/IfcDirection.h"

#include "OpenInfraPlatform/IfcAlignment/guid/CreateGuid_64.h"
#include "UnitConverter.h"
#include "IfcAlignmentP6Exception.h"
#include "IfcAlignmentP6Model.h"
...
//             ref_direction->m_DirectionRatios.push_back( 1.0 );
		...
		shared_ptr<IfcProject> IfcAlignmentModel::getIfcProject()
		{
			return m_ifc_project;
		}
		shared_ptr<IfcGeometricRepresentationContext> IfcAlignmentModel::getIfcGeometricRepresentationContext3D()
		{
			return m_geom_context_3d;
		}

		void IfcAlignmentModel::setIfcProject( shared_ptr<IfcProject> project )
		{
		...
		void IfcAlignmentModel::removeEntity( shared_ptr<IfcAlignmentP6Entity> e )
		{
			int remove_id = e->getId();
			std::map<int,shared_ptr<IfcAlignmentP6Entity> >::iterator it_find = m_map_entities.find(remove_id);
			if( it_find != m_map_entities.end() )
			{
				shared_ptr<IfcAlignmentP6Entity> entity_found( it_find->second );
				entity_found->unlinkSelf();
				m_map_entities.erase( it_find );
			}
		}

		void IfcAlignmentModel::removeEntity( int entity_id )
		{
			std::map<int,shared_ptr<IfcAlignmentP6Entity> >::iterator it_find = m_map_entities.find(entity_id);
			if( it_find != m_map_entities.end() )
			{
				shared_ptr<IfcAlignmentP6Entity> entity_found( it_find->second );
				entity_found->unlinkSelf();
		...
		int IfcAlignmentModel::getMaxUsedEntityId()
		{
			...
			std::map<int,shared_ptr<IfcAlignmentP6Entity> >::iterator it_entities = m_map_entities.end();
			--it_entities;
		...
		void IfcAlignmentModel::initFileHeader( std::string file_name )
		{
			std::stringstream strs;
		...
		void IfcAlignmentModel::setFileHeader( std::string header )
		{
			m_file_header = header;
		}

		void IfcAlignmentModel::setFileSchema( std::string schema )
		{
			m_file_schema = schema;
		}

		void IfcAlignmentModel::clearIfcModel()
		{
			m_ifc_project.reset();
			m_geom_context_3d.reset();

			std::map<int,shared_ptr<IfcAlignmentP6Entity> >::iterator it_entities;
			for( it_entities = m_map_entities.begin(); it_entities != m_map_entities.end(); ++it_entities )
			{
				shared_ptr<IfcAlignmentP6Entity> entity( it_entities->second );
				entity->unlinkSelf();
			}
		...
		void IfcAlignmentModel::updateCache()
		{
			bool found_project = false;
			bool found_geom_context = false;

			// try to find IfcProject and IfcGeometricRepresentationContext
			std::map<int,shared_ptr<IfcAlignmentP6Entity> >::iterator it;
			std::map<int,shared_ptr<IfcAlignmentP6Entity> >::iterator it_end = m_map_entities.end();
		...
		void IfcAlignmentModel::clearCache()
		{
			m_ifc_project.reset();
			m_geom_context_3d.reset();
		}

		void IfcAlignmentModel::resolveInverseAttributes()
		{
