...
#include "IfcTunnelEntitiesMap.h"
...
namespace OpenInfraPlatform
{
    namespace IfcTunnel
    {
        static std::tr1::unordered_map<std::string,IfcTunnelEntityEnum> map_string2entity_enum(initializers_IfcTunnel_entity, initializers_IfcTunnel_entity + sizeof(initializers_IfcTunnel_entity)/sizeof(initializers_IfcTunnel_entity[0]));
        void applyBackwardCompatibility( IfcTunnelModel::IfcVersion backward_version, IfcTunnelEntityEnum type_enum, std::vector<std::string>& args );
        void applyBackwardCompatibility( std::string& keyword, std::string& step_line );
        IfcTunnelEntity* createIfcTunnelEntity( const IfcTunnelEntityEnum entity_enum );

        void readStepLine( const std::string& line, shared_ptr<IfcTunnelEntity>& entity )
        {
            ...
            if( keyword.size() > 0 )
            {
                //std::map<std::string,IfcTunnelEntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                std::tr1::unordered_map<std::string,IfcTunnelEntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                ...
                {
                    IfcTunnelEntityEnum entity_enum = it_entity_enum->second;

                    shared_ptr<IfcTunnelEntity> obj( createIfcTunnelEntity( entity_enum ) );
                    if( obj )
                    {
                        obj->setId( id );
                        entity = obj;
        ...
        void IfcStepReader::readStreamHeader( std::string& read_in, shared_ptr<IfcTunnelModel> model )
        {
            ...
            if( file_schema_begin != std::string::npos )
            {
                ...
                if( file_schema_end != std::string::npos )
                {
                    ...
                    {
                        m_ifc_version = IfcTunnelModel::UNKNOWN;
                    }
                }
            }
        }
        ...
        void IfcStepReader::readStepLines( const std::vector<std::string>& step_lines, std::vector<shared_ptr<IfcTunnelEntity> >& target_entity_vec )
        {
...
#ifdef IfcTunnel_OPENMP
            std::vector<shared_ptr<IfcTunnelEntity> >* target_vec_ptr = &target_entity_vec;
            ...
            #pragma omp parallel firstprivate(num_lines) shared(target_vec_ptr,unkown_entities_ptr)
            {
                ...
                for( int i=0; i<num_lines; ++i )
                {
                    ...
                    try
                    {
                        shared_ptr<IfcTunnelEntity> entity;
        ...
        void IfcStepReader::readEntityArguments( const std::vector<shared_ptr<IfcTunnelEntity> >& vec_entities, const std::map<int,shared_ptr<IfcTunnelEntity> > map_entities )
        {
            ...
            const std::map<int,shared_ptr<IfcTunnelEntity> >* map_ptr = &map_entities;
            const std::vector<shared_ptr<IfcTunnelEntity> >* vec_entities_ptr = &vec_entities;

            #pragma omp parallel firstprivate(num_objects) shared(map_ptr,vec_entities_ptr)
            {
                const std::map<int,shared_ptr<IfcTunnelEntity> > &map_loc = *map_ptr;
                std::stringstream thread_err;

                #pragma omp for schedule(dynamic, 100)
                for( int i=0; i<num_objects; ++i )
                {
                    shared_ptr<IfcTunnelEntity> entity = vec_entities_ptr->at(i);
                    std::string& argument_str = entity->m_arguments;
                    std::vector<std::string> arguments;
                    tokenizeEntityArguments( argument_str, arguments );

                    if( m_ifc_version != IfcTunnelModel::IFC4 )
                    {
                        if( m_ifc_version != IfcTunnelModel::UNDEFINED && m_ifc_version != IfcTunnelModel::UNKNOWN )
                        {
                            IfcTunnelEntityEnum entity_enum = entity->m_entity_enum;
            ...
            std::vector<shared_ptr<IfcTunnelEntity> >::const_iterator it_entity_vec = vec_entities.begin();
            int i=0;
            for( ; it_entity_vec!=vec_entities.end(); ++it_entity_vec )
            {
                shared_ptr<IfcTunnelEntity> entity = (*it_entity_vec);
                std::string& argument_str = entity->m_arguments;
                std::vector<std::string> arguments;
                tokenizeEntityArguments( argument_str, arguments );
...
#endif // IfcTunnel_OPENMP

            if( err.tellp() > 0 )
            {
                throw IfcTunnelException( err.str() );
            }
        }

        void IfcStepReader::readStreamData(	std::string& read_in, std::map<int,shared_ptr<IfcTunnelEntity> >& target_map )
        {
            ...
            std::vector<shared_ptr<IfcTunnelEntity> > vec_entities;
            try
            {
        ...
        void applyBackwardCompatibility( IfcTunnelModel::IfcVersion version, IfcTunnelEntityEnum type_enum, std::vector<std::string>& args )
        {
            // TODO: replace this workaround with a systematic backward compatibility, possibly generated from schema diff
            if( version < IfcTunnelModel::IFC2X )
            {
                throw IfcTunnelException( "Unsupported IFC version" );
            }
            if( version <= IfcTunnelModel::IFC2X )
            {
            ...
            if( version <= IfcTunnelModel::IFC2X2 )
            {
                switch( type_enum )
                {
                case IFCCOLOURRGB:
            ...
            if( version <= IfcTunnelModel::IFC2X3 )
            {
                switch( type_enum )
                {
                // B
            ...
            if( version <= IfcTunnelModel::IFC4 )
            {
                switch( type_enum )
                {
                // I
