...
namespace OpenInfraPlatform
{
	namespace IfcTunnel
	{
		...
		void IfcDimensionalExponents::setEntity( shared_ptr<IfcTunnelEntity> other_entity )
		{
		...
		void IfcDimensionalExponents::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcTunnelEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<7 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcDimensionalExponents, expecting 7, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcTunnelException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcDimensionalExponents::setInverseCounterparts( shared_ptr<IfcTunnelEntity> )
		{
		}
