...
namespace OpenInfraPlatform
{
	namespace IfcTunnel
	{
		...
		void IfcPerson::setEntity( shared_ptr<IfcTunnelEntity> other_entity )
		{
		...
		void IfcPerson::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcTunnelEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPerson, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcTunnelException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcPerson::setInverseCounterparts( shared_ptr<IfcTunnelEntity> ptr_self_entity )
		{
			shared_ptr<IfcPerson> ptr_self = dynamic_pointer_cast<IfcPerson>( ptr_self_entity );
			if( !ptr_self ) { throw IfcTunnelException( "IfcPerson::setInverseCounterparts: type mismatch" ); }
