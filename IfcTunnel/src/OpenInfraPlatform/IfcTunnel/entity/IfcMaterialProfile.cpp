...
namespace OpenInfraPlatform
{
	namespace IfcTunnel
	{
		...
		void IfcMaterialProfile::setEntity( shared_ptr<IfcTunnelEntity> other_entity )
		{
		...
		void IfcMaterialProfile::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcTunnelEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcMaterialProfile, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcTunnelException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcMaterialProfile::setInverseCounterparts( shared_ptr<IfcTunnelEntity> ptr_self_entity )
		{
