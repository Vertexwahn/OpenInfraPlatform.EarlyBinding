...
namespace OpenInfraPlatform
{
	namespace IfcTunnel
	{
		...
		void IfcTaskType::setEntity( shared_ptr<IfcTunnelEntity> other_entity )
		{
		...
		void IfcTaskType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcTunnelEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTaskType, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcTunnelException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcTaskType::setInverseCounterparts( shared_ptr<IfcTunnelEntity> ptr_self_entity )
		{
