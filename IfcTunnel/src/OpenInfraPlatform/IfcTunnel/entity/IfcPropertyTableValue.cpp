...
namespace OpenInfraPlatform
{
	namespace IfcTunnel
	{
		...
		void IfcPropertyTableValue::setEntity( shared_ptr<IfcTunnelEntity> other_entity )
		{
		...
		void IfcPropertyTableValue::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcTunnelEntity> >& map )
		{
			...
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPropertyTableValue, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcTunnelException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcPropertyTableValue::setInverseCounterparts( shared_ptr<IfcTunnelEntity> ptr_self_entity )
		{
