...
namespace OpenInfraPlatform
{
	namespace IfcTunnel
	{
		...
		void IfcExternalReference::setEntity( shared_ptr<IfcTunnelEntity> other_entity )
		{
		...
		void IfcExternalReference::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcTunnelEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<3 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcExternalReference, expecting 3, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcTunnelException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcExternalReference::setInverseCounterparts( shared_ptr<IfcTunnelEntity> )
		{
		}
