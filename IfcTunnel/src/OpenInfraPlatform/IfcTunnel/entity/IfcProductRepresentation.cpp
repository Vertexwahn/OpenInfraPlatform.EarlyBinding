...
namespace OpenInfraPlatform
{
	namespace IfcTunnel
	{
		...
		void IfcProductRepresentation::setEntity( shared_ptr<IfcTunnelEntity> other_entity )
		{
		...
		void IfcProductRepresentation::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcTunnelEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<3 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcProductRepresentation, expecting 3, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcTunnelException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcProductRepresentation::setInverseCounterparts( shared_ptr<IfcTunnelEntity> ptr_self_entity )
		{
			shared_ptr<IfcProductRepresentation> ptr_self = dynamic_pointer_cast<IfcProductRepresentation>( ptr_self_entity );
			if( !ptr_self ) { throw IfcTunnelException( "IfcProductRepresentation::setInverseCounterparts: type mismatch" ); }
