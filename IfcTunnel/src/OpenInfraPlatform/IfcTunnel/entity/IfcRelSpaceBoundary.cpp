...
namespace OpenInfraPlatform
{
	namespace IfcTunnel
	{
		...
		void IfcRelSpaceBoundary::setEntity( shared_ptr<IfcTunnelEntity> other_entity )
		{
		...
		void IfcRelSpaceBoundary::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcTunnelEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelSpaceBoundary, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcTunnelException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelSpaceBoundary::setInverseCounterparts( shared_ptr<IfcTunnelEntity> ptr_self_entity )
		{
			IfcRelConnects::setInverseCounterparts( ptr_self_entity );
			shared_ptr<IfcRelSpaceBoundary> ptr_self = dynamic_pointer_cast<IfcRelSpaceBoundary>( ptr_self_entity );
			if( !ptr_self ) { throw IfcTunnelException( "IfcRelSpaceBoundary::setInverseCounterparts: type mismatch" ); }
