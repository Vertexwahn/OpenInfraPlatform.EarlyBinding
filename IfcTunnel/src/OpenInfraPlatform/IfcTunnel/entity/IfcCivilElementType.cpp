...
namespace OpenInfraPlatform
{
	namespace IfcTunnel
	{
		...
		void IfcCivilElementType::setEntity( shared_ptr<IfcTunnelEntity> other_entity )
		{
		...
		void IfcCivilElementType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcTunnelEntity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<9 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcCivilElementType, expecting 9, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcTunnelException( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcCivilElementType::setInverseCounterparts( shared_ptr<IfcTunnelEntity> ptr_self_entity )
		{
