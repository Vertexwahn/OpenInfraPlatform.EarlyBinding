...
#include "IfcTunnelModel.h"

namespace OpenInfraPlatform
{
    namespace IfcTunnel
    {
        IfcTunnelModel::IfcTunnelModel()
        {
            m_unit_converter = shared_ptr<UnitConverter>( new UnitConverter() );
            initFileHeader( "IfcTunnel-export.ifc" );
        }
        ...
        void IfcTunnelModel::initIfcModel()
        {
            clearIfcModel();
            ...
            app->m_ApplicationFullName = shared_ptr<IfcLabel>( new IfcLabel( "IfcTunnel" ) );
            app->m_ApplicationIdentifier = shared_ptr<IfcIdentifier>( new IfcIdentifier( "IfcTunnel" ) );
            insertEntity(app);
            ...
            project->m_GlobalId = shared_ptr<IfcGloballyUniqueId>(new IfcGloballyUniqueId( CreateCompressedGuidString() ) );
            project->m_OwnerHistory = owner_history;
            project->m_Name = shared_ptr<IfcLabel>(new IfcLabel( "IfcTunnel project" ) );
        ...
        shared_ptr<IfcProject> IfcTunnelModel::getIfcProject()
        {
            return m_ifc_project;
        }
        shared_ptr<IfcGeometricRepresentationContext> IfcTunnelModel::getIfcGeometricRepresentationContext3D()
        {
            return m_geom_context_3d;
        }

        void IfcTunnelModel::setIfcProject( shared_ptr<IfcProject> project )
        {
            m_ifc_project = project;
        }

        void IfcTunnelModel::insertEntity( shared_ptr<IfcTunnelEntity> e, bool overwrite_existing )
        {
            ...
            std::map<int,shared_ptr<IfcTunnelEntity> >::iterator lb = m_map_entities.lower_bound( entity_id );
            ...
            if( lb != m_map_entities.end() && !(m_map_entities.key_comp()(entity_id, lb->first)) )
            {
                ...
                else
                {
                    ...
                    strs << "IfcTunnelModel::insertEntity: entity with id " << entity_id << " already exists in model" << std::endl;
                    throw IfcTunnelException( strs.str().c_str() );
                }
            }
            else
            {
                // the key does not exist in the map
                m_map_entities.insert( lb, std::map<int,shared_ptr<IfcTunnelEntity> >::value_type( entity_id, e ) );
            }

            // TODO: if type is IfcRoot (or subtype), and GlobalID not set, create one
        }

        void IfcTunnelModel::removeEntity( shared_ptr<IfcTunnelEntity> e )
        {
            int remove_id = e->getId();
            std::map<int,shared_ptr<IfcTunnelEntity> >::iterator it_find = m_map_entities.find(remove_id);
            if( it_find != m_map_entities.end() )
            {
                shared_ptr<IfcTunnelEntity> entity_found( it_find->second );
                entity_found->unlinkSelf();
                m_map_entities.erase( it_find );
            }
        }

        void IfcTunnelModel::removeEntity( int entity_id )
        {
            std::map<int,shared_ptr<IfcTunnelEntity> >::iterator it_find = m_map_entities.find(entity_id);
            if( it_find != m_map_entities.end() )
            {
                shared_ptr<IfcTunnelEntity> entity_found( it_find->second );
                entity_found->unlinkSelf();
        ...
        int IfcTunnelModel::getMaxUsedEntityId()
        {
            ...
            std::map<int,shared_ptr<IfcTunnelEntity> >::iterator it_entities = m_map_entities.end();
            --it_entities;
        ...
        void IfcTunnelModel::initFileHeader( std::string file_name )
        {
            std::stringstream strs;
        ...
        void IfcTunnelModel::setFileHeader( std::string header )
        {
            m_file_header = header;
        }

        void IfcTunnelModel::setFileSchema( std::string schema )
        {
            m_file_schema = schema;
        }

        void IfcTunnelModel::clearIfcModel()
        {
            m_ifc_project.reset();
            m_geom_context_3d.reset();

            std::map<int,shared_ptr<IfcTunnelEntity> >::iterator it_entities;
            for( it_entities = m_map_entities.begin(); it_entities != m_map_entities.end(); ++it_entities )
            {
                shared_ptr<IfcTunnelEntity> entity( it_entities->second );
                entity->unlinkSelf();
            }
            m_map_entities.clear();
        }

        void IfcTunnelModel::resetIfcModel()
        {
            clearIfcModel();
            initIfcModel();
            updateCache();
        ...
        void IfcTunnelModel::updateCache()
        {
            bool found_project = false;
            bool found_geom_context = false;

            // try to find IfcProject and IfcGeometricRepresentationContext
            std::map<int,shared_ptr<IfcTunnelEntity> >::iterator it;
            std::map<int,shared_ptr<IfcTunnelEntity> >::iterator it_end = m_map_entities.end();
        ...
        void IfcTunnelModel::clearCache()
        {
            m_ifc_project.reset();
            m_geom_context_3d.reset();
        }

        void IfcTunnelModel::resolveInverseAttributes()
        {
