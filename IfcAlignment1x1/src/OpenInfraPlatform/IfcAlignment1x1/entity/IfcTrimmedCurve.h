...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		public:
			...
			// IfcTrimmedCurve -----------------------
			// attributes:
			std::shared_ptr<IfcCurve> m_BasisCurve;
			std::vector<std::shared_ptr<IfcTrimmingSelect>>  m_Trim1;
			std::vector<std::shared_ptr<IfcTrimmingSelect>>  m_Trim2;
			std::shared_ptr<IfcBoolean> m_SenseAgreement;
			std::shared_ptr<IfcTrimmingPreference> m_MasterRepresentation;
		};
	}
}

VISITABLE_STRUCT(OpenInfraPlatform::IfcAlignment1x1::IfcTrimmedCurve, m_BasisCurve, m_Trim1, m_Trim2, m_SenseAgreement, m_MasterRepresentation);
