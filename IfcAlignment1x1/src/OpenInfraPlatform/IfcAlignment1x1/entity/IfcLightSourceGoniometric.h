...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		public:
			...
			// IfcLightSourceGoniometric -----------------------
			// attributes:
			std::shared_ptr<IfcAxis2Placement3D> m_Position;
			std::shared_ptr<IfcColourRgb> m_ColourAppearance;
			std::shared_ptr<IfcThermodynamicTemperatureMeasure> m_ColourTemperature;
			std::shared_ptr<IfcLuminousFluxMeasure> m_LuminousFlux;
			std::shared_ptr<IfcLightEmissionSourceEnum> m_LightEmissionSource;
			std::shared_ptr<IfcLightDistributionDataSourceSelect> m_LightDistributionDataSource;
		};
	}
}
