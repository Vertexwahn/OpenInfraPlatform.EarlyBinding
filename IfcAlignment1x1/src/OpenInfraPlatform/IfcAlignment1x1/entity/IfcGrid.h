...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		public:
			...
			// IfcGrid -----------------------
			// attributes:
			std::vector<std::shared_ptr<IfcGridAxis>>  m_UAxes;
			std::vector<std::shared_ptr<IfcGridAxis>>  m_VAxes;
			std::vector<std::shared_ptr<IfcGridAxis>>  m_WAxes;
			std::shared_ptr<IfcGridTypeEnum> m_PredefinedType;
		};
	}
}

VISITABLE_STRUCT(OpenInfraPlatform::IfcAlignment1x1::IfcGrid, m_GlobalId, m_OwnerHistory, m_Name, m_Description, m_ObjectType, m_ObjectPlacement, m_Representation, m_UAxes, m_VAxes, m_WAxes, m_PredefinedType);
