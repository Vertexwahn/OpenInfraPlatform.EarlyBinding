...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		public:
			...
			// IfcSurfaceTexture -----------------------
			// attributes:
			std::shared_ptr<IfcBoolean> m_RepeatS;
			std::shared_ptr<IfcBoolean> m_RepeatT;
			std::shared_ptr<IfcIdentifier> m_Mode;
			std::shared_ptr<IfcCartesianTransformationOperator2D> m_TextureTransform;
			std::vector<std::shared_ptr<IfcIdentifier>>  m_Parameter;
		};
	}
}

VISITABLE_STRUCT(OpenInfraPlatform::IfcAlignment1x1::IfcSurfaceTexture, m_RepeatS, m_RepeatT, m_Mode, m_TextureTransform, m_Parameter);
