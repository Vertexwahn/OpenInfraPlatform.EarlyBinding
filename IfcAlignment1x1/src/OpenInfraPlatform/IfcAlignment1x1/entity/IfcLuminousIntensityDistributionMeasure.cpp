...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcLuminousIntensityDistributionMeasure> IfcLuminousIntensityDistributionMeasure::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcLuminousIntensityDistributionMeasure>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcLuminousIntensityDistributionMeasure>(); }
			std::shared_ptr<IfcLuminousIntensityDistributionMeasure> type_object(new IfcLuminousIntensityDistributionMeasure() );
