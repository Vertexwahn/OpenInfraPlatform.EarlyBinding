...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		public:
			...
			// IfcBSplineCurveWithKnots -----------------------
			// attributes:
			std::vector<std::shared_ptr<IfcInteger>>  m_KnotMultiplicities;
			std::vector<std::shared_ptr<IfcParameterValue>>  m_Knots;
			std::shared_ptr<IfcKnotType> m_KnotSpec;
		};
	}
}

VISITABLE_STRUCT(OpenInfraPlatform::IfcAlignment1x1::IfcBSplineCurveWithKnots, m_Degree, m_ControlPointsList, m_CurveForm, m_ClosedCurve, m_SelfIntersect, m_KnotMultiplicities, m_Knots, m_KnotSpec);
