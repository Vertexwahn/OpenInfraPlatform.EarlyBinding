...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcMonthInYearNumber> IfcMonthInYearNumber::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcMonthInYearNumber>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcMonthInYearNumber>(); }
			std::shared_ptr<IfcMonthInYearNumber> type_object(new IfcMonthInYearNumber() );
