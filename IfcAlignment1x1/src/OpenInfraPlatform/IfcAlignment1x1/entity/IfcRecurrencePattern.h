...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		class IfcRecurrencePattern : public IfcAlignment1x1Entity
		{
		public:
			...
			// IfcRecurrencePattern -----------------------
			// attributes:
			std::shared_ptr<IfcRecurrenceTypeEnum> m_RecurrenceType;
			std::vector<std::shared_ptr<IfcDayInMonthNumber>>  m_DayComponent;
			std::vector<std::shared_ptr<IfcDayInWeekNumber>>  m_WeekdayComponent;
			std::vector<std::shared_ptr<IfcMonthInYearNumber>>  m_MonthComponent;
			std::shared_ptr<IfcInteger> m_Position;
			std::shared_ptr<IfcInteger> m_Interval;
			std::shared_ptr<IfcInteger> m_Occurrences;
			std::vector<std::shared_ptr<IfcTimePeriod>>  m_TimePeriods;
		};
	}
}
