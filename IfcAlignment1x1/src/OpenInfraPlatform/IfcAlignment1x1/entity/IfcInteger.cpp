...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcInteger> IfcInteger::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcInteger>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcInteger>(); }
			std::shared_ptr<IfcInteger> type_object(new IfcInteger() );
