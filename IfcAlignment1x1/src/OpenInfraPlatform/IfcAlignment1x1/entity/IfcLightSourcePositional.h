...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		public:
			...
			// IfcLightSourcePositional -----------------------
			// attributes:
			std::shared_ptr<IfcCartesianPoint> m_Position;
			std::shared_ptr<IfcPositiveLengthMeasure> m_Radius;
			std::shared_ptr<IfcReal> m_ConstantAttenuation;
			std::shared_ptr<IfcReal> m_DistanceAttenuation;
			std::shared_ptr<IfcReal> m_QuadricAttenuation;
		};
	}
}

VISITABLE_STRUCT(OpenInfraPlatform::IfcAlignment1x1::IfcLightSourcePositional, m_Name, m_LightColour, m_AmbientIntensity, m_Intensity, m_Position, m_Radius, m_ConstantAttenuation, m_DistanceAttenuation, m_QuadricAttenuation);
