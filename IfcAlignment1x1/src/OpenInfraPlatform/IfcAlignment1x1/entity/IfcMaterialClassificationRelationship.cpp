...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcMaterialClassificationRelationship::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcMaterialClassificationRelationship> other = std::dynamic_pointer_cast<IfcMaterialClassificationRelationship>(other_entity);
				if( !other) { return; }
			...
			void IfcMaterialClassificationRelationship::getStepLine( std::stringstream& stream ) const
			{
				...
				writeTypeList( stream, m_MaterialClassifications );
				stream << ",";
			...
			void IfcMaterialClassificationRelationship::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcMaterialClassificationRelationship, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcMaterialClassificationRelationship::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
			}
