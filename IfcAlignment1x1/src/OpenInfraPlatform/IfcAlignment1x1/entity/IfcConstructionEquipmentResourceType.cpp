...
#include "IfcConstructionEquipmentResourceType.h"
...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcConstructionEquipmentResourceType::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcConstructionEquipmentResourceType> other = std::dynamic_pointer_cast<IfcConstructionEquipmentResourceType>(other_entity);
				if( !other) { return; }
			...
			void IfcConstructionEquipmentResourceType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<12 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcConstructionEquipmentResourceType, expecting 12, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
				if( num_args<12 ){ std::cout << "Wrong parameter count for entity IfcConstructionEquipmentResourceType, expecting 12, having " << num_args << ". Object id: " << getId() << std::endl; }
				#endif
				...
				m_PredefinedType = IfcConstructionEquipmentResourceTypeEnum::readStepData( args[11] );
			}
			void IfcConstructionEquipmentResourceType::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
