...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcConstructionMaterialResource::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcConstructionMaterialResource> other = std::dynamic_pointer_cast<IfcConstructionMaterialResource>(other_entity);
				if( !other) { return; }
			...
			void IfcConstructionMaterialResource::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcConstructionMaterialResource, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
				...
				m_PredefinedType = IfcConstructionMaterialResourceTypeEnum::readStepData( args[10] );
			}
			void IfcConstructionMaterialResource::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
