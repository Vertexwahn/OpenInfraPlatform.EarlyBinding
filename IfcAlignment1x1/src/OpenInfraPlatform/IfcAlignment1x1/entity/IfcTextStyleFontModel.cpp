...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcTextStyleFontModel::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcTextStyleFontModel> other = std::dynamic_pointer_cast<IfcTextStyleFontModel>(other_entity);
				if( !other) { return; }
				m_Name = other->m_Name;
			...
			void IfcTextStyleFontModel::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_FontSize) { m_FontSize->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcTextStyleFontModel::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcTextStyleFontModel::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTextStyleFontModel, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcTextStyleFontModel::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
