...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		public:
			...
			// IfcCurrencyRelationship -----------------------
			// attributes:
			std::shared_ptr<IfcMonetaryUnit> m_RelatingMonetaryUnit;
			std::shared_ptr<IfcMonetaryUnit> m_RelatedMonetaryUnit;
			std::shared_ptr<IfcPositiveRatioMeasure> m_ExchangeRate;
			std::shared_ptr<IfcDateTime> m_RateDateTime;
			std::shared_ptr<IfcLibraryInformation> m_RateSource;
		};
	}
}

VISITABLE_STRUCT(OpenInfraPlatform::IfcAlignment1x1::IfcCurrencyRelationship, m_Name, m_Description, m_RelatingMonetaryUnit, m_RelatedMonetaryUnit, m_ExchangeRate, m_RateDateTime, m_RateSource);
