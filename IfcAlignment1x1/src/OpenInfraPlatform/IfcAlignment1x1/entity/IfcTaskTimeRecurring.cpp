...
#include "IfcTaskTimeRecurring.h"
#include "IfcLabel.h"
...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcTaskTimeRecurring::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcTaskTimeRecurring> other = std::dynamic_pointer_cast<IfcTaskTimeRecurring>(other_entity);
				if( !other) { return; }
				m_Name = other->m_Name;
			...
			void IfcTaskTimeRecurring::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_Recurrence) { stream << "#" << m_Recurrence->getId(); } else { stream << "$"; }
				stream << ");";
			}
			void IfcTaskTimeRecurring::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcTaskTimeRecurring::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<21 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTaskTimeRecurring, expecting 21, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcTaskTimeRecurring::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
