...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcRelDeclares::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcRelDeclares> other = std::dynamic_pointer_cast<IfcRelDeclares>(other_entity);
				if( !other) { return; }
			...
			void IfcRelDeclares::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				writeTypeList( stream, m_RelatedDefinitions );
				stream << ");";
			}
			void IfcRelDeclares::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcRelDeclares::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelDeclares, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcRelDeclares::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
