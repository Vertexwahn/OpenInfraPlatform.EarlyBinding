...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcDoor::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcDoor> other = std::dynamic_pointer_cast<IfcDoor>(other_entity);
				if( !other) { return; }
			...
			void IfcDoor::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<13 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcDoor, expecting 13, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
				if( num_args<13 ){ std::cout << "Wrong parameter count for entity IfcDoor, expecting 13, having " << num_args << ". Object id: " << getId() << std::endl; }
				#endif
			...
			void IfcDoor::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
				IfcBuildingElement::setInverseCounterparts(ptr_self_entity);
			}
			void IfcDoor::unlinkSelf()
			{
