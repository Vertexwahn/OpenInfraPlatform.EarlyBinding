...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		IfcSegmentIndexSelect::~IfcSegmentIndexSelect() {}
		std::shared_ptr<IfcSegmentIndexSelect> IfcSegmentIndexSelect::readStepData(const std::string& arg, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map)
		{
			// Read SELECT TYPE
			if( arg.size() == 0 ) { return std::shared_ptr<IfcSegmentIndexSelect>(); }
			if( arg[0] == '#' )
			{
