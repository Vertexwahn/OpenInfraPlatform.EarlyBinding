...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcCurveStyle::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcCurveStyle> other = std::dynamic_pointer_cast<IfcCurveStyle>(other_entity);
				if( !other) { return; }
				m_Name = other->m_Name;
			...
			void IfcCurveStyle::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_CurveFont) { m_CurveFont->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_CurveWidth) { m_CurveWidth->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_CurveColour) { m_CurveColour->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
			...
			void IfcCurveStyle::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<5 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcCurveStyle, expecting 5, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcCurveStyle::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
