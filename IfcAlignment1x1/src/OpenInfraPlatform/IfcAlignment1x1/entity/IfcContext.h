...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		public:
			...
			// IfcContext -----------------------
			// attributes:
			std::shared_ptr<IfcLabel> m_ObjectType;
			std::shared_ptr<IfcLabel> m_LongName;
			std::shared_ptr<IfcLabel> m_Phase;
			std::vector<std::shared_ptr<IfcRepresentationContext>>  m_RepresentationContexts;
			std::shared_ptr<IfcUnitAssignment> m_UnitsInContext;
		};
	}
}

VISITABLE_STRUCT(OpenInfraPlatform::IfcAlignment1x1::IfcContext, m_GlobalId, m_OwnerHistory, m_Name, m_Description, m_ObjectType, m_LongName, m_Phase, m_RepresentationContexts, m_UnitsInContext);
