...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		std::shared_ptr<IfcDayInWeekNumber> IfcDayInWeekNumber::readStepData(const std::string& arg)
		{
			if( arg.compare( "$" ) == 0 ) { return std::shared_ptr<IfcDayInWeekNumber>(); }
			else if( arg.compare( "*" ) == 0 ) { return std::shared_ptr<IfcDayInWeekNumber>(); }
			std::shared_ptr<IfcDayInWeekNumber> type_object(new IfcDayInWeekNumber() );
