...
#include "IfcClothoidalArcSegment2D.h"
...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcClothoidalArcSegment2D::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcClothoidalArcSegment2D> other = std::dynamic_pointer_cast<IfcClothoidalArcSegment2D>(other_entity);
				if( !other) { return; }
			...
			void IfcClothoidalArcSegment2D::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<7 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcClothoidalArcSegment2D, expecting 7, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcClothoidalArcSegment2D::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
