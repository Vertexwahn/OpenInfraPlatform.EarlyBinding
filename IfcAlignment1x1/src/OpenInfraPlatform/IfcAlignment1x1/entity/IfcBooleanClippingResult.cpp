...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcBooleanClippingResult::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcBooleanClippingResult> other = std::dynamic_pointer_cast<IfcBooleanClippingResult>(other_entity);
				if( !other) { return; }
			...
			void IfcBooleanClippingResult::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_FirstOperand) { m_FirstOperand->getStepParameter( stream ); } else { stream << "$"; }
				stream << ",";
				if( m_SecondOperand) { m_SecondOperand->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcBooleanClippingResult::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcBooleanClippingResult::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<3 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcBooleanClippingResult, expecting 3, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcBooleanClippingResult::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
