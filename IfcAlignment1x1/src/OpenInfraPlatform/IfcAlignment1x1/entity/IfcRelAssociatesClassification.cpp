...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcRelAssociatesClassification::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcRelAssociatesClassification> other = std::dynamic_pointer_cast<IfcRelAssociatesClassification>(other_entity);
				if( !other) { return; }
			...
			void IfcRelAssociatesClassification::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_RelatingClassification) { m_RelatingClassification->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcRelAssociatesClassification::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcRelAssociatesClassification::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelAssociatesClassification, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcRelAssociatesClassification::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
