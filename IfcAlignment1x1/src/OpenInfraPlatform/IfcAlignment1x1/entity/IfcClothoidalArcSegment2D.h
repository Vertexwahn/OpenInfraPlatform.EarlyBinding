...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
		...
		public:
			...
			// IfcClothoidalArcSegment2D -----------------------
			// attributes:
			std::shared_ptr<IfcPositiveLengthMeasure> m_StartRadius;
			std::shared_ptr<IfcBoolean> m_IsCCW;
			std::shared_ptr<IfcBoolean> m_IsEntry;
			std::shared_ptr<IfcReal> m_ClothoidConstant;
		};
	}
}

VISITABLE_STRUCT(OpenInfraPlatform::IfcAlignment1x1::IfcClothoidalArcSegment2D, m_StartPoint, m_StartDirection, m_SegmentLength, m_StartRadius, m_IsCCW, m_IsEntry, m_ClothoidConstant);
