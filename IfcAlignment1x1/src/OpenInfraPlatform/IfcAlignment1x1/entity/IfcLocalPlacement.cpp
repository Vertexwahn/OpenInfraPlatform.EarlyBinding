...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcLocalPlacement::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcLocalPlacement> other = std::dynamic_pointer_cast<IfcLocalPlacement>(other_entity);
				if( !other) { return; }
			...
			void IfcLocalPlacement::getStepLine( std::stringstream& stream ) const
			{
				...
				stream << ",";
				if( m_RelativePlacement) { m_RelativePlacement->getStepParameter( stream ); } else { stream << "$"; }
				stream << ");";
			}
			void IfcLocalPlacement::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
			void IfcLocalPlacement::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcLocalPlacement, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
			...
			void IfcLocalPlacement::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
