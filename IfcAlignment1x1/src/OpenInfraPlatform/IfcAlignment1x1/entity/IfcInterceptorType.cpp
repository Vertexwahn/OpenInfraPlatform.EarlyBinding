...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcInterceptorType::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcInterceptorType> other = std::dynamic_pointer_cast<IfcInterceptorType>(other_entity);
				if( !other) { return; }
			...
			void IfcInterceptorType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcInterceptorType, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
				if( num_args<10 ){ std::cout << "Wrong parameter count for entity IfcInterceptorType, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; }
				#endif
				...
				m_PredefinedType = IfcInterceptorTypeEnum::readStepData( args[9] );
			}
			void IfcInterceptorType::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
