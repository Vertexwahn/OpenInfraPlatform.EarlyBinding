...
namespace OpenInfraPlatform
{
	namespace IfcAlignment1x1
	{
			...
			void IfcPolygonalFaceSet::setEntity( std::shared_ptr<IfcAlignment1x1Entity> other_entity)
			{
				std::shared_ptr<IfcPolygonalFaceSet> other = std::dynamic_pointer_cast<IfcPolygonalFaceSet>(other_entity);
				if( !other) { return; }
			...
			void IfcPolygonalFaceSet::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcAlignment1x1Entity> >& map )
			{
				const int num_args = (int)args.size();
				if( num_args<4 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPolygonalFaceSet, expecting 4, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcAlignment1x1Exception( strserr.str().c_str() ); }
				#ifdef _DEBUG
				if( num_args<4 ){ std::cout << "Wrong parameter count for entity IfcPolygonalFaceSet, expecting 4, having " << num_args << ". Object id: " << getId() << std::endl; }
				#endif
				readEntityReference( args[0], m_Coordinates, map);
				m_Closed = IfcBoolean::readStepData( args[1] );
				readEntityReferenceList( args[2], m_Faces , map);
				readTypeList( args[3], m_PnIndex );
			}
			void IfcPolygonalFaceSet::setInverseCounterparts(shared_ptr<IfcAlignment1x1Entity> ptr_self_entity)
			{
				IfcTessellatedFaceSet::setInverseCounterparts(ptr_self_entity);
			}
			void IfcPolygonalFaceSet::unlinkSelf()
			{
