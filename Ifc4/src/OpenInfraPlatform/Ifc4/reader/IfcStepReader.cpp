...
namespace OpenInfraPlatform
{
    namespace Ifc4
    {
        static std::unordered_map<std::string,Ifc4EntityEnum> map_string2entity_enum(initializers_Ifc4_entity, initializers_Ifc4_entity + sizeof(initializers_Ifc4_entity)/sizeof(initializers_Ifc4_entity[0]));
        void applyBackwardCompatibility( Ifc4Model::IfcVersion backward_version, Ifc4EntityEnum type_enum, std::vector<std::string>& args );
        void applyBackwardCompatibility( std::string& keyword, std::string& step_line );
        Ifc4Entity* createIfc4Entity( const Ifc4EntityEnum entity_enum );

        void readStepLine( const std::string& line, shared_ptr<Ifc4Entity>& entity )
        {
            ...
            if( keyword.size() > 0 )
            {
                //std::map<std::string,Ifc4EntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                std::unordered_map<std::string,Ifc4EntityEnum>::iterator it_entity_enum = map_string2entity_enum.find( keyword );
                ...
                {
                    Ifc4EntityEnum entity_enum = it_entity_enum->second;

                    shared_ptr<Ifc4Entity> obj( createIfc4Entity( entity_enum ) );
                    if( obj )
                    {
                        obj->setId( id );
                        entity = obj;
        ...
        void IfcStepReader::readStreamHeader( std::string& read_in, shared_ptr<Ifc4Model> model )
        {
            ...
            if( file_schema_begin != std::string::npos )
            {
                ...
                if( file_schema_end != std::string::npos )
                {
                    ...
                    {
                        m_ifc_version = Ifc4Model::IFC2X2;
                    }
                    else if( m_file_schema.substr(0,6).compare("IFC4") == 0 )
                    {
                        m_ifc_version = Ifc4Model::Ifc4;
                    }
                    else if( m_file_schema.substr(0,6).compare("IFC2X4") == 0 )
                    {
                        m_ifc_version = Ifc4Model::IFC2X4;
                    }
                    else if( m_file_schema.substr(0,5).compare("IFC2X") == 0 )
                    {
                        m_ifc_version = Ifc4Model::IFC2X;
                    }
                    ...
                    {
                        m_ifc_version = Ifc4Model::UNKNOWN;
                    }
                }
            }
        }
        ...
        void IfcStepReader::readStepLines( const std::vector<std::string>& step_lines, std::vector<shared_ptr<Ifc4Entity> >& target_entity_vec )
        {
...
#ifdef Ifc4_OPENMP
            std::vector<shared_ptr<Ifc4Entity> >* target_vec_ptr = &target_entity_vec;
        ...
        void IfcStepReader::readEntityArguments( const std::vector<shared_ptr<Ifc4Entity> >& vec_entities, const std::map<int,shared_ptr<Ifc4Entity> > map_entities )
        {
            ...
            const std::map<int,shared_ptr<Ifc4Entity> >* map_ptr = &map_entities;
            const std::vector<shared_ptr<Ifc4Entity> >* vec_entities_ptr = &vec_entities;

            #pragma omp parallel firstprivate(num_objects) shared(map_ptr,vec_entities_ptr)
            {
                const std::map<int,shared_ptr<Ifc4Entity> > &map_loc = *map_ptr;
                std::stringstream thread_err;

                #pragma omp for schedule(dynamic, 100)
                for( int i=0; i<num_objects; ++i )
                {
                    shared_ptr<Ifc4Entity> entity = vec_entities_ptr->at(i);
                    std::string& argument_str = entity->m_arguments;
                    std::vector<std::string> arguments;
                    tokenizeEntityArguments( argument_str, arguments );

                    if( m_ifc_version != Ifc4Model::Ifc4 )
                    {
                        if( m_ifc_version != Ifc4Model::UNDEFINED && m_ifc_version != Ifc4Model::UNKNOWN )
                        {
                            Ifc4EntityEnum entity_enum = entity->m_entity_enum;
            ...
            std::vector<shared_ptr<Ifc4Entity> >::const_iterator it_entity_vec = vec_entities.begin();
            int i=0;
            for( ; it_entity_vec!=vec_entities.end(); ++it_entity_vec )
            {
                shared_ptr<Ifc4Entity> entity = (*it_entity_vec);
            ...
            if( err.tellp() > 0 )
            {
                throw Ifc4Exception( err.str() );
            }
        }

        void IfcStepReader::readStreamData(	std::string& read_in, std::map<int,shared_ptr<Ifc4Entity> >& target_map )
        {
            ...
            std::vector<shared_ptr<Ifc4Entity> > vec_entities;
            try
            {
        ...
        void applyBackwardCompatibility( Ifc4Model::IfcVersion version, Ifc4EntityEnum type_enum, std::vector<std::string>& args )
        {
            // TODO: replace this workaround with a systematic backward compatibility, possibly generated from schema diff
            if( version < Ifc4Model::IFC2X )
            {
                throw Ifc4Exception( "Unsupported IFC version" );
            }
            if( version <= Ifc4Model::IFC2X )
            {
