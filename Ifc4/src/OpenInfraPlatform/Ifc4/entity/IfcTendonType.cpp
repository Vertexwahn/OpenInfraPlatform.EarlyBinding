...
namespace OpenInfraPlatform
{
	namespace Ifc4
	{
		...
		void IfcTendonType::setEntity( shared_ptr<Ifc4Entity> other_entity )
		{
		...
		void IfcTendonType::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc4Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<13 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTendonType, expecting 13, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc4Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcTendonType::setInverseCounterparts( shared_ptr<Ifc4Entity> ptr_self_entity )
		{
