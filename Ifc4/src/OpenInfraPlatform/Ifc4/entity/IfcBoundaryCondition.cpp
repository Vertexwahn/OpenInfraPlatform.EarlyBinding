...
namespace OpenInfraPlatform
{
	namespace Ifc4
	{
		...
		void IfcBoundaryCondition::setEntity( shared_ptr<Ifc4Entity> other_entity )
		{
		...
		void IfcBoundaryCondition::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc4Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<1 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcBoundaryCondition, expecting 1, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc4Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcBoundaryCondition::setInverseCounterparts( shared_ptr<Ifc4Entity> )
		{
		}
