...
namespace OpenInfraPlatform
{
	namespace Ifc4
	{
		...
		void IfcAxis2Placement2D::setEntity( shared_ptr<Ifc4Entity> other_entity )
		{
		...
		void IfcAxis2Placement2D::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc4Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<2 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcAxis2Placement2D, expecting 2, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc4Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcAxis2Placement2D::setInverseCounterparts( shared_ptr<Ifc4Entity> ptr_self_entity )
		{
