...
namespace OpenInfraPlatform
{
	namespace Ifc4
	{
		...
		void IfcRelCoversSpaces::setEntity( shared_ptr<Ifc4Entity> other_entity )
		{
		...
		void IfcRelCoversSpaces::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc4Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<6 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelCoversSpaces, expecting 6, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc4Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelCoversSpaces::setInverseCounterparts( shared_ptr<Ifc4Entity> ptr_self_entity )
		{
			IfcRelConnects::setInverseCounterparts( ptr_self_entity );
			shared_ptr<IfcRelCoversSpaces> ptr_self = dynamic_pointer_cast<IfcRelCoversSpaces>( ptr_self_entity );
			if( !ptr_self ) { throw Ifc4Exception( "IfcRelCoversSpaces::setInverseCounterparts: type mismatch" ); }
