...
namespace OpenInfraPlatform
{
	namespace Ifc4
	{
		...
		void IfcTimeSeries::setEntity( shared_ptr<Ifc4Entity> other_entity )
		{
		...
		void IfcTimeSeries::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc4Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<8 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcTimeSeries, expecting 8, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc4Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcTimeSeries::setInverseCounterparts( shared_ptr<Ifc4Entity> )
		{
		}
