...
namespace OpenInfraPlatform
{
	namespace Ifc4
	{
		...
		void IfcPostalAddress::setEntity( shared_ptr<Ifc4Entity> other_entity )
		{
		...
		void IfcPostalAddress::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc4Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcPostalAddress, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc4Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcPostalAddress::setInverseCounterparts( shared_ptr<Ifc4Entity> ptr_self_entity )
		{
