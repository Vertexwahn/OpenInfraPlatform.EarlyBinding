...
namespace OpenInfraPlatform
{
	namespace Ifc4
	{
		...
		void IfcRelDefines::setEntity( shared_ptr<Ifc4Entity> other_entity )
		{
		...
		void IfcRelDefines::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<Ifc4Entity> >& map )
		{
			const int num_args = (int)args.size();
			if( num_args<4 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcRelDefines, expecting 4, having " << num_args << ". Object id: " << getId() << std::endl; throw Ifc4Exception( strserr.str().c_str() ); }
			#ifdef _DEBUG
		...
		void IfcRelDefines::setInverseCounterparts( shared_ptr<Ifc4Entity> ptr_self_entity )
		{
